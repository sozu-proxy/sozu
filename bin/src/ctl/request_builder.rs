...
impl CommandManager {
    ...
    pub fn dump_state(&mut self, json: bool) -> anyhow::Result<()> {
        ...
        self.order_command_with_json(Order::DumpState, json)
    }
    ...
    pub fn soft_stop(&mut self, worker_id: Option<u32>) -> anyhow::Result<()> {
        ...
        self.order_command_with_worker_id(
            Order::Worker(Box::new(WorkerOrder::SoftStop)),
            worker_id,
            false,
        )
    }
    ...
    pub fn hard_stop(&mut self, worker_id: Option<u32>) -> anyhow::Result<()> {
        ...
        self.order_command_with_worker_id(
            Order::Worker(Box::new(WorkerOrder::HardStop)),
            worker_id,
            false,
        )
    }
    ...
    */
    ...
    pub fn status(&mut self, json: bool) -> anyhow::Result<()> {
        ...
        self.order_command_with_json(Order::Status, json)
    }
    ...
    pub fn configure_metrics(&mut self, cmd: MetricsCmd) -> anyhow::Result<()> {
        ...
        self.order_command(Order::Worker(Box::new(WorkerOrder::ConfigureMetrics(
            configuration,
        ))))
    }
    ...
    pub fn reload_configuration(&mut self, path: Option<String>, json: bool) -> anyhow::Result<()> {
        ...
        self.order_command_with_json(Order::ReloadConfiguration { path }, json)
    }
    ...
    pub fn backend_command(&mut self, cmd: BackendCmd) -> anyhow::Result<()> {
        match cmd {
            BackendCmd::Add {
                ...
                sticky_id,
                backup,
            } => self.order_command(Order::Worker(Box::new(WorkerOrder::AddBackend(Backend {
                cluster_id: id,
                address,
                backend_id,
            ...
            BackendCmd::Remove {
                ...
                backend_id,
                address,
            } => self.order_command(Order::Worker(Box::new(WorkerOrder::RemoveBackend(
                RemoveBackend {
                    cluster_id: id,
                    address,
                    backend_id,
    ...
    pub fn cluster_command(&mut self, cmd: ClusterCmd) -> anyhow::Result<()> {
        match cmd {
            ClusterCmd::Add {
                ...
                self.order_command(Order::Worker(Box::new(WorkerOrder::AddCluster(Cluster {
                    cluster_id: id,
                    sticky_session,
                    https_redirect,
                    proxy_protocol,
            ...
            ClusterCmd::Remove { id } => {
                self.order_command(Order::Worker(Box::new(WorkerOrder::RemoveCluster {
                    cluster_id: id,
                })))
            }
        }
    ...
    pub fn tcp_frontend_command(&mut self, cmd: TcpFrontendCmd) -> anyhow::Result<()> {
        match cmd {
            TcpFrontendCmd::Add { id, address, tags } => self.order_command(Order::Worker(
                Box::new(WorkerOrder::AddTcpFrontend(TcpFrontend {
                    cluster_id: id,
                    address,
                    tags,
                })),
            )),
            TcpFrontendCmd::Remove { id, address } => self.order_command(Order::Worker(Box::new(
                WorkerOrder::RemoveTcpFrontend(TcpFrontend {
                    cluster_id: id,
                    address,
                    tags: None,
                }),
    ...
    pub fn https_listener_command(&mut self, cmd: HttpsListenerCmd) -> anyhow::Result<()> {
        match cmd {
            HttpsListenerCmd::Add {
                ...
                self.order_command(Order::Worker(Box::new(WorkerOrder::AddHttpsListener(
                    https_listener,
                ))))
            }
    ...
    pub fn http_listener_command(&mut self, cmd: HttpListenerCmd) -> anyhow::Result<()> {
        match cmd {
            HttpListenerCmd::Add {
                ...
                self.order_command(Order::Worker(Box::new(WorkerOrder::AddHttpListener(
                    http_listener,
                ))))
            }
    ...
    pub fn tcp_listener_command(&mut self, cmd: TcpListenerCmd) -> anyhow::Result<()> {
        match cmd {
            TcpListenerCmd::Add {
                ...
                public_address,
                expect_proxy,
            } => self.order_command(Order::Worker(Box::new(WorkerOrder::AddTcpListener(
                TcpListenerConfig {
                    address,
                    public_address,
                    expect_proxy,
    ...
    pub fn remove_listener(
        ...
        self.order_command(Order::Worker(Box::new(WorkerOrder::RemoveListener(
            RemoveListener { address, proxy },
        ))))
    }
    ...
    pub fn activate_listener(
        ...
        self.order_command(Order::Worker(Box::new(WorkerOrder::ActivateListener(
            ActivateListener {
                address,
                proxy,
                from_scm: false,
    ...
    pub fn deactivate_listener(
        ...
        self.order_command(Order::Worker(Box::new(WorkerOrder::DeactivateListener(
            DeactivateListener {
                address,
                proxy,
                to_scm: false,
    ...
    pub fn logging_filter(&mut self, filter: &LoggingLevel) -> anyhow::Result<()> {
        self.order_command(Order::Worker(Box::new(WorkerOrder::Logging(
    ...
    pub fn replace_certificate(
        ...
        self.order_command(Order::Worker(Box::new(WorkerOrder::ReplaceCertificate(
            ReplaceCertificate {
                address,
                new_certificate,
                old_fingerprint,
    ...
    pub fn remove_certificate(
        ...
        self.order_command(Order::Worker(Box::new(WorkerOrder::RemoveCertificate(
            RemoveCertificate {
                address,
                fingerprint,
            },
