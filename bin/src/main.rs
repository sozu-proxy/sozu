...
use worker::{get_executable_path, start_workers};
...
fn main() {
    ...
    if upgrade == None {
        ...
        match start {
            ...
            Err(StartupError::TooManyAllowedConnections(err))
            | Err(StartupError::TooManyAllowedConnectionsForWorker(err)) => {
                error!("{}", err);
                std::process::exit(1);
            }
...
fn check_process_limits(config: &Config) -> Result<(), StartupError> {
    ...
    if let Some(hard_limit) = re
        .captures(&process_limits_file)
        .and_then(|c| c.get(1))
