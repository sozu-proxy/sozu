    ...
    App::new("sozu")
            ...
            SubCommand::with_name("worker")
                ...
                .arg(
                    Arg::with_name("scm")
                        ...
                        .takes_value(true)
                        .required(true)
                        .help("IPC SCM_RIGHTS file descriptor"),
                )
                .arg(
                    Arg::with_name("configuration-state-fd")
                ...
                .arg(
                    Arg::with_name("command-buffer-size")
                ...
                .arg(
                    Arg::with_name("max-command-buffer-size")
        ...
        .subcommand(
            SubCommand::with_name("upgrade")
                ...
                .arg(
                    Arg::with_name("upgrade-fd")
                        .long("upgrade-fd")
                        .takes_value(true)
                        .required(true)
                        .help("upgrade data file descriptor"),
                )
                .arg(
                    Arg::with_name("command-buffer-size")
                        .long("command-buffer-size")
                        .takes_value(true)
                        .required(true)
                        .help("Master's command buffer size"),
                )
                .arg(
                    Arg::with_name("max-command-buffer-size")
                        .long("max-command-buffer-size")
                        .takes_value(true)
                        .required(false)
                        .help("Master's max command buffer size"),
                ),
        )
        .get_matches()
}
...
pub fn get_fd<'a>(matches: &ArgMatches<'a>) -> i32 {
    matches
        .value_of("fd")
        .expect("needs a file descriptor")
        .parse::<i32>()
        .expect("the file descriptor must be a number")
}
...
pub fn get_scm<'a>(matches: &ArgMatches<'a>) -> i32 {
    matches
        ...
        .expect("needs a file descriptor")
        .parse::<i32>()
        .expect("the SCM_RIGHTS file descriptor must be a number")
}

pub fn get_configuration_state_fd<'a>(matches: &ArgMatches<'a>) -> i32 {
    matches
        .value_of("configuration-state-fd")
...
pub fn get_id<'a>(matches: &ArgMatches<'a>) -> i32 {
    matches
        .value_of("id")
        .expect("needs a worker id")
        .parse::<i32>()
        .expect("the worker id must be a number")
}

pub fn get_buffer_size<'a>(matches: &ArgMatches<'a>) -> usize {
    matches
        .value_of("command-buffer-size")
...
pub fn get_max_buffer_size<'a>(matches: &ArgMatches<'a>, buffer_size: usize) -> usize {
    matches
        .value_of("max-command-buffer-size")
        .and_then(|size| size.parse::<usize>().ok())
...
pub fn get_upgrade_fd<'a>(matches: &ArgMatches<'a>) -> i32 {
    matches
        .value_of("upgrade-fd")
        .expect("needs an upgrade file descriptor")
        .parse::<i32>()
        .expect("the file descriptor must be a number")
}
