...
use sozu_command::proxy::{
    AggregatedMetricsData, HttpFront, MetricsData, ProxyRequest, ProxyRequestData,
    ProxyResponseData, Query, QueryAnswer, QueryApplicationType, TcpFront,
};
...
use util;
use worker::{get_executable_path, start_worker};
...
impl CommandServer {
    ...
    {
        trace!(
            "answer_success for front token {:?} id {}, message {:#?} data {:#?}",
            token,
            id.clone().into(),
            message.clone().into(),
            data
    ...
    {
        trace!(
            "answer_error for front token {:?} id {}, message {:#?} data {:#?}",
            token,
            id.clone().into(),
            message.clone().into(),
            data
                    ...
                    match parse(buffer.data()) {
                        Ok((i, orders)) => {
                            ...
                            for order in diff {
                                ...
                                for ref mut worker in self.workers.values_mut().filter(|worker| {
                                    worker.run_state != RunState::Stopping
                                        && worker.run_state != RunState::Stopped
                                }) {
                                    let o = order.clone();
                                    futures.push(executor::send(
    ...
    pub fn launch_worker(&mut self, token: FrontToken, message: &CommandRequest, tag: &str) {
        ...
        if let Ok(mut worker) = start_worker(
            ...
            debug!(
                "registering new sock {:?} at token {:?} for tag {} and id {} (sock error: {:?})",
                worker.channel.sock,
                worker_token,
                tag,
                worker.id,
                worker.channel.sock.take_error()
            );
            self.poll
                .register(
                    &worker.channel.sock,
    ...
    pub fn upgrade_worker(&mut self, token: FrontToken, message_id: &str, id: u32) {
        ...
        let mut listeners = None;
        {
            ...
            old_worker.channel.write_message(&ProxyRequest {
                id: String::from(message_id),
                order: ProxyRequestData::ReturnListenSockets,
            });
            ...
            executor::Executor::execute(
                executor::send(
                    ...
                    ProxyRequest {
                        ...
                        order: ProxyRequestData::SoftStop,
                    },
                )
                .map(move |_| executor::Executor::stop_worker(old_worker_token))
                .map_err(|s| {
    ...
    pub fn upgrade_master(&mut self, token: FrontToken, message_id: &str) {
        ...
        self.clients[token]
            .channel
            .write_message(&CommandResponse::new(
                String::from(message_id),
                CommandStatus::Processing,
                "".to_string(),
                None,
        ...
        for ref mut worker in self.workers.values_mut().filter(|worker| {
            ...
            futures.push(
                executor::send(
                    ...
                    ProxyRequest {
                        id: id.clone(),
                        order: ProxyRequestData::Metrics,
                    },
                )
                .map(|data| (tag, data)),
            );
    ...
    pub fn query(&mut self, token: FrontToken, message_id: &str, query: Query) {
        ...
        for ref mut worker in self.workers.values_mut().filter(|worker| {
            ...
            futures.push(
                executor::send(
                    ...
                    ProxyRequest {
                        id: id.clone(),
                        order: ProxyRequestData::Query(query.clone()),
                    },
                )
                .map(|data| (tag, data)),
            );
        ...
        let f = join_all(futures).map(move |v| {
            let data: BTreeMap<String, QueryAnswer> = v
                .into_iter()
            ...
            &Query::Applications(ref query_type) => {
                let master = match query_type {
                    ...
                    QueryApplicationType::Domain(ref domain) => {
                        ...
                        app_ids
                            .iter()
                            .map(|ref app_id| self.state.application_state(app_id))
                            .collect()
                    }
                };
    ...
    pub fn worker_order(
        ...
        executor::Executor::execute(
            f.map(move |_v| {
                executor::Executor::send_client(
                    token,
                    CommandResponse::new(id, CommandStatus::Ok, String::new(), None),
                );
            })
            .map_err(|e| {
