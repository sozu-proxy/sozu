...
use sozu_command::command::{
    self, CommandRequest, CommandResponse, CommandResponseData, CommandStatus, RunState,
};
...
impl Worker {
    ...
    pub fn can_handle_events(&self) -> bool {
        self.channel.readiness().is_readable()
            || (!self.queue.is_empty() && self.channel.readiness().is_writable())
    }
}
...
impl CommandServer {
    fn accept(&mut self) -> io::Result<()> {
        ...
        if let Ok(Some((sock, _))) = acc {
            ...
            self.poll
                .reregister(&self.sock, SERVER, accept_interest, PollOpt::edge())
        } else {
    ...
    fn new(
        ...
        METRICS.with(|metrics| {
            if let Some(sock) = (*metrics.borrow()).socket() {
                poll.register(sock, Token(1), Ready::writable(), PollOpt::edge())
...
impl CommandServer {
    pub fn run(&mut self) {
        ...
        loop {
            ...
            loop {
                let mut did_something = false;
                {
                    let tokens: Vec<Token> = self
                        .clients
                        .iter()
                        .filter(|client| client.can_handle_events())
                        .map(|client| client.token.unwrap())
                        .collect();
...

                {
                    let tokens: Vec<Token> = self
                        .workers
                        .iter()
                        .filter(|&(_, worker)| worker.can_handle_events())
            ...
            let clients_not_served = self.clients.iter().filter(|c| !c.queue.is_empty()).count();
        ...
        while let Some(state_change) = Executor::get_state_change() {
            match state_change {
                StateChange::StopWorker(token) => {
                    self.workers
                        .get_mut(&token)
                        .map(|w| w.run_state = RunState::Stopped);
                }
    ...
    pub fn handle_worker_events(&mut self, token: Token) {
        ...
        let worker_run_state = self
            .workers
            .get(&token)
            .as_ref()
            .map(|worker| worker.run_state);
        if (self.config.worker_automatic_restart
            && worker_run_state == Some(RunState::NotAnswering))
            || worker_run_state == Some(RunState::Stopping)
        {
    ...
    pub fn handle_client_events(&mut self, conn_token: FrontToken) {
        if self.clients.contains(conn_token) {
            if UnixReady::from(self.clients[conn_token].channel.readiness).is_hup() {
                let _ = self
                    .poll
                    .deregister(&self.clients[conn_token].channel.sock)
                    .map_err(|e| {
    ...
    pub fn check_worker_status(&mut self, token: Token) {
        ...
        if self.config.worker_automatic_restart {
            ...
            if let Ok(mut worker) = start_worker(
                ...
                debug!(
                    "registering new sock {:?} at token {:?} for id {} (sock error: {:?})",
                    worker.channel.sock,
                    worker_token,
                    worker.id,
