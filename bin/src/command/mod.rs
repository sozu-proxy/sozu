...
use sozu_command_lib::{
    ...
    state::ConfigState,
    worker::{
        MetricsConfiguration, WorkerOrder, WorkerRequest, WorkerResponse, WorkerResponseStatus,
    },
};
        ...
        match self.in_flight.remove(&response.id) {
            ...
            Some((mut requester_tx, mut expected_responses)) => {
                ...
                match response.status {
                    WorkerResponseStatus::Ok | WorkerResponseStatus::Error(_) => {
...
/// - parse WorkerResponses from the unix stream and send them to the CommandServer
