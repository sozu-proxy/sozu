...
The probe wire format (HTTP/1.1 vs h2c) is derived automatically from the cluster's `http2` flag — there is no separate `is_h2c` knob on the health-check config. Setting `[clusters.<id>] http2 = true` switches both the data-plane backend connection and the health-check probe to HTTP/2 prior-knowledge in lockstep, so an h2c-only backend is never probed with HTTP/1.1 (and vice versa).
