...
- **HTTP/1.1 only — no h2c probing**: Health checks send a plain-text HTTP/1.1 request over TCP. Clusters configured with `http2 = true` (cleartext HTTP/2 backends, h2c) will fail probes because the server expects an HTTP/2 connection preface, not HTTP/1.1. Workarounds: (a) co-locate an HTTP/1.1 health endpoint on the same backend, (b) leave `health_check` unset for h2c-only clusters and rely on connection-level health, or (c) wait for HTTP/2 prior-knowledge probe support (tracked as a follow-up). HTTPS probes are also not yet implemented; backends that only accept TLS connections fail health checks today.
