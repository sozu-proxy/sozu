...
| `command_allowed_uids`     | optional allowlist of POSIX UIDs permitted to invoke command-socket requests (PR #1209 / Lisa LISA-007). Omitted or `None` preserves the historical "any same-UID local process" behaviour. Set to `[<operator_uid>]` to restrict mutating verbs to a specific UID even when other same-UID daemons coexist (CI runners, monitoring scrapers). Rejected requests still appear in the audit trail. | TOML array of integers, e.g. `[1000]` |
