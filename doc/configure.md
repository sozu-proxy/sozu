...
| `http.frontend.transfer_encoding_smuggling`  | counter | proxy | H1 request rejected (400) for ambiguous `Transfer-Encoding` framing: more than one non-elided `Transfer-Encoding` header, or one whose raw value does not literally end in `chunked` (e.g. the OWS-obfuscated `chunked\t` / `chunked `, or the non-final `chunked, gzip`), or one present without kawa adopting chunked framing (RFC 9110 §7.6 / RFC 9112 §6.1; #726)                                      |
