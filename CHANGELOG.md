...
- **`feat(udp)`: first-class UDP listeners with load balancing** ([#1273](https://github.com/sozu-proxy/sozu/pull/1273)).
  ...
  - **Testing:** a moonpool/VOPR-style **deterministic simulation harness** (`lib/tests/udp_simulation.rs`) drives the pure sans-io UDP core through a seeded, adversarial workload (weighted action grammar + FDB-`buggify` fault injection + a 256-seed sweep), layered on dense `debug_assert` invariants in `manager.rs`/`flow.rs`; failures reproduce from a printed seed via `SOZU_UDP_SIM_SEED`. See [`doc/udp_simulation.md`](doc/udp_simulation.md).
