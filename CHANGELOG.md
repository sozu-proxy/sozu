...
- **Active HTTP/1.1 backend health checks with fail-open ([#1191](https://github.com/sozu-proxy/sozu/pull/1191))**: new `lib/src/health_check.rs` runs probes inside the existing single-threaded mio event loop (no extra threads, no async runtime). Probes use a bounded token namespace `[1<<24, 1<<24 + 1<<16)` with a modulo-allocator that skips in-flight slots and refuses allocation rather than colliding with the mux GOAWAY sentinel `Token(usize::MAX)`. Threshold-based state machine, jittered intervals, 4 KB response buffer cap, CRLF/NUL/C0-stripped URIs. Fail-open routing in `lib/src/load_balancing.rs`: when ALL backends for a cluster are unhealthy, route to Normal backends rather than returning 503. New CLI verbs `sozu cluster health-check {set,remove,list}`. Configurable per-cluster via `[clusters.<id>.health_check]` (TOML) or `Cluster.health_check` (proto field 15). New events `HEALTH_CHECK_HEALTHY = 27` / `HEALTH_CHECK_UNHEALTHY = 28` and metrics `health_check.{success,failure,up,down,healthy_backends}`. Known limitation: probes are HTTP/1.1 only, so backends with `http2 = true` (h2c) require either a co-located HTTP/1.1 health endpoint or no `health_check` configuration. See `doc/health_checks.md`.
