...
- **Dedicated audit log sink — `audit_logs_target` config**: new optional path (in `[General]`, next to `log_target` / `access_logs_target`). When set, every audit line is also appended to the file opened `O_APPEND | O_CREAT | chmod 0640` so the audit trail can be separated from the main log stream and protected via `audit`-group ACL (PCI-DSS 10.5 "protect audit trails"). ANSI escape sequences are stripped from the dedicated-sink copy so the file stays SIEM-parseable even when colour is enabled on stdout. Best-effort: write failures log an error but never block the mutation. `None` (default) preserves the original routing through `log_target`.
