...
- **Hot-upgrade race against on-disk binary replacement ([#515](https://github.com/sozu-proxy/sozu/issues/515))**: `bin/src/upgrade.rs` (master hot-upgrade re-exec) and `bin/src/worker.rs` (worker spawn / auto-restart) previously called `Command::new(executable_path).exec()` where `executable_path` was a string captured at master startup via `read_link("/proc/self/exe")`. If an operator replaced the on-disk binary at that path between master startup and the exec call (`cp new-sozu /usr/bin/sozu` followed by `sozuctl upgrade`, or a worker auto-restart triggered after an in-place package upgrade), `execve(2)` resolved the path string as a normal filesystem lookup and started the **new** binary, incompatible with the running master's protocol expectations. The new helper `crate::util::get_executable_exec_path()` opens `/proc/self/exe` with `O_PATH | O_CLOEXEC` and returns `/proc/self/fd/<n>` for `execve(2)` to resolve through the magic symlink to the **original** inode the master was started from (`O_CLOEXEC` closes the fd on successful exec; on failed exec the fd persists harmlessly for the master's lifetime). Linux-only via `cfg(target_os = "linux")`; FreeBSD / macOS keep the path-string approach (out of scope for #515 / v2.0.0). Both exec sites fail-soft on `O_PATH` open failure (log + fall back to path-string exec). New unit tests in `bin/src/util.rs::tests` (`get_executable_exec_path_returns_proc_self_fd_on_linux`, `get_executable_exec_path_distinct_calls_yield_distinct_fds`) cover the helper's shape; an e2e upgrade-race test in `e2e/src/tests/upgrade_race_tests.rs` covering binary-replacement-during-upgrade is a follow-up.
