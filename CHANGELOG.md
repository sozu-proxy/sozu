...
- **`LICENSE-LGPL3` shipped at the workspace root**: matches the repo-level `LICENSE` (AGPL-3.0) and lets the release tarballs and source archives carry the canonical LGPL-3.0 text used by `sozu-command-lib` rather than pointing at an external URL. The corresponding-source `SOURCE.txt` in each release tarball now references both license files in-archive.
