...
use sozu_command::config::{Config, LoadBalancingAlgorithms, ProxyProtocolConfig};
use sozu_command::proxy::{
    AddCertificate, Application, Backend, CertFingerprint, CertificateAndKey, HttpFront,
    LoadBalancingParams, ProxyRequestData, Query, QueryAnswer, QueryAnswerCertificate,
    QueryApplicationDomain, QueryApplicationType, QueryCertificateType, RemoveBackend,
    RemoveCertificate, ReplaceCertificate, TcpFront,
};
...
pub fn upgrade_master(mut channel: Channel<CommandRequest, CommandResponse>, config: &Config) {
                ...
                CommandStatus::Ok => {
                    if let Some(CommandResponseData::Workers(ref workers)) = message.data {
                            ...
                            match channel.read_message() {
                                ...
                                Some(message) => {
                                    ...
                                    match message.status {
                                        ...
                                        CommandStatus::Error => {
                                            eprintln!(
                                                "Error: failed to upgrade the master: {}",
                                                message.message
                                            );
                                            exit(1);
                                        }
                                        CommandStatus::Ok => {
                                            println!(
                                                "Master process upgrade succeeded: {}",
                                                message.message
                                            );
                                            break;
                                        }
                        ...
                        let mut channel = create_channel(&config)
                            .expect("could not reconnect to the command unix socket");
...
pub fn upgrade_worker(
    ...
    timeout: u64,
    worker_id: u32,
) -> Channel<CommandRequest, CommandResponse> {
    ...
    timeout_thread
        .join()
        .expect("upgrade_worker: Timeout thread should correctly terminate")
}
                    ...
                    CommandStatus::Ok => {
                        if &id == &message.id {
                            ...
                            if let Some(CommandResponseData::Metrics(data)) = message.data {
                                ...
                                master_table
                                    .add_row(row![String::from("key"), String::from("value")]);

                                for (ref key, ref value) in data.master.iter() {
                                    master_table
                                        .add_row(row![key.to_string(), format!("{:?}", value)]);
                                }
                                ...
                                for app_id in app_ids.iter() {
                                    ...
                                    for app_metric in app_metrics.iter() {
                                        ...
                                        for worker in data.workers.values() {
                                            row.push(cell!(worker
                                                .applications
                                                .get(id)
                                                .and_then(|app| app.data.get(metric))
                                                .map(|value| format!("{:?}", value))
                                                .unwrap_or(String::new())));
                                        }
                                    ...
                                    for backend_id in backend_ids.iter() {
                                        ...
                                        backend_table.add_row(row![format!(
                                            "app: {} - backend: {}",
                                            id, backend
                                        )]);
                                        ...
                                        for backend_metric in backend_metrics.iter() {
                                            ...
                                            for worker in data.workers.values() {
                                                row.push(cell!(worker
                                                    .applications
                                                    .get(id)
                                                    .and_then(|app| app.backends.get(backend))
...
pub fn remove_application(
    ...
    order_command(
        channel,
        timeout,
        ProxyRequestData::RemoveApplication(String::from(app_id)),
    );
}
...
pub fn add_http_frontend(
    ...
    if https {
        order_command(
            channel,
            timeout,
            ProxyRequestData::AddHttpsFront(HttpFront {
                app_id: String::from(app_id),
                address,
...
pub fn add_certificate(
    ...
    if let Some(new_certificate) =
        load_full_certificate(certificate_path, certificate_chain_path, key_path)
    {
        order_command(
            channel,
            timeout,
    ...
    if let Some(new_certificate) = load_full_certificate(
        ...
        if let Some(old_fingerprint) = get_certificate_fingerprint(old_certificate_path) {
            order_command(
                channel,
                timeout,
                ProxyRequestData::ReplaceCertificate(ReplaceCertificate {
                    front: address,
                    new_certificate,
                    old_fingerprint,
                    new_names: Vec::new(),
...
pub fn add_tcp_frontend(
    ...
    order_command(
        channel,
        timeout,
        ProxyRequestData::AddTcpFront(TcpFront {
            app_id: String::from(app_id),
            address,
        }),
    );
...
pub fn query_application(
    ...
    let command = if let Some(ref app_id) = application_id {
        CommandRequestData::Proxy(ProxyRequestData::Query(Query::Applications(
            QueryApplicationType::AppId(app_id.to_string()),
        )))
