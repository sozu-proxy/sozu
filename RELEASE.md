...
After the tag triggers `.github/workflows/release.yml`, twelve `sozu-${VERSION}-${TARGET}-${PROVIDER}.tar.gz` archives plus a `SHA256SUMS` file (signed with sigstore cosign keyless — `SHA256SUMS.sig` + `SHA256SUMS.pem`) land on a GitHub draft release alongside SLSA build provenance. The matrix covers four targets (`x86_64-unknown-linux-{gnu,musl}`, `aarch64-unknown-linux-{gnu,musl}`) crossed with four crypto providers — `crypto-ring` (default, all 4 targets), `crypto-aws-lc-rs` (all 4 targets), `crypto-openssl` (gnu targets only — `rustls-openssl` needs a vendored OpenSSL on musl), and `fips` (gnu targets only — `aws-lc-fips-sys` precompiled FIPS module assumes glibc). Each tarball contains the stripped `sozu` binary, `LICENSE-AGPL3`, `LICENSE-LGPL3`, `README.md`, `CHANGELOG.md`, the production-shaped `os-build/config.toml`, both shipped systemd units, and a `SOURCE.txt` corresponding-source pointer that records the target and crypto provider for the cell.
