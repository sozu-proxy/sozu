...
impl HealthChecker {
    ...
    fn allocate_token(&mut self) -> Token {
        let token = Token(HEALTH_CHECK_TOKEN_BASE + self.next_token_id);
    ...
    fn initiate_checks(
        ...
        for (cluster_id, config) in &backend_map.health_check_configs {
            ...
            let jitter_ms =
                hasher.finish() % (interval.as_millis() as u64 / 5).max(1);
                ...
                match TcpStream::connect(address) {
                    Ok(mut stream) => {
                        ...
                        if let Err(e) = registry.register(
                            ...
                            debug!(
                                "health check: failed to register socket for {} ({}) in cluster {}: {}",
    ...
    fn progress_checks(
        ...
        for (idx, check) in self.in_flight.iter_mut().enumerate() {
            ...
            match check.stream.read(&mut buf) {
                Ok(0) => {
                    let success =
                        try_parse_status_line(&check.response_buf, &check.config).unwrap_or(false);
