...
use sozu_command::proxy::{
    AddCertificate, Application, HttpFront, HttpsListener, ProxyRequest, ProxyRequestData,
    ProxyResponse, ProxyResponseData, ProxyResponseStatus, Query, QueryAnswer,
    QueryAnswerCertificate, QueryCertificateType, RemoveCertificate, ReplaceCertificate,
    TlsVersion,
};
...
use pool::Pool;
use protocol::http::parser::{hostname_and_port, RRequestLine};
use protocol::http::DefaultAnswerStatus;
use protocol::StickySession;
use server::{
    ListenPortState, ListenSession, ListenToken, ProxyChannel, Server, SessionToken, CONN_RETRIES,
};
use socket::server_bind;
use trie::*;
use util::UnwrapLog;
use {
    AcceptError, AppId, BackendConnectAction, BackendConnectionStatus, ConnectionError, Protocol,
    ProxyConfiguration, ProxySession,
};
...
impl Listener {
    ...
    pub fn add_https_front(&mut self, tls_front: HttpFront) -> bool {
        ...
        if let Some((_, fronts)) = self
            .fronts
            .domain_lookup_mut(&tls_front.hostname.as_bytes())
        {
        ...
        if self
            .fronts
            .domain_lookup(&tls_front.hostname.as_bytes())
            .is_none()
        {
            self.fronts
        ...
        match message.order {
            ...
            ProxyRequestData::ReplaceCertificate(replace_certificate) => {
                ...
                if let Some(listener) = self
                    .listeners
                    .values_mut()
                    .find(|l| l.address == replace_certificate.front)
                {
