...
use sozu_command::proxy::{LoadBalancingParams, ProxyRequest, ProxyResponse};
...
use self::server::{ListenPortState, ListenToken};
...
impl Backend {
    ...
    pub fn try_connect(&mut self) -> Result<mio::tcp::TcpStream, ConnectionError> {
        ...
        let conn = mio::tcp::TcpStream::connect(&self.address)
