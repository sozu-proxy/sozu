...
impl Server {
    pub fn new_from_config(
        ...
        let pool = Rc::new(RefCell::new(Pool::with_capacity(
            2 * config.max_buffers,
            0,
            || Buffer::with_capacity(config.buffer_size),
        )));
        ...
        let mut sessions: Slab<Rc<RefCell<ProxySessionCast>>, SessionToken> =
            Slab::with_capacity(10 + 2 * config.max_connections);
        {
            let entry = sessions
                .vacant_entry()
    ...
    pub fn new(
        ...
        METRICS.with(|metrics| {
            if let Some(sock) = (*metrics.borrow()).socket() {
                poll.register(sock, Token(2), Ready::writable(), PollOpt::edge())
        ...
        let mut server = Server {
            ...
            zombie_check_interval: time::Duration::seconds(i64::from(
                server_config.zombie_check_interval,
            )),
            accept_queue: VecDeque::new(),
            accept_queue_timeout: time::Duration::seconds(i64::from(
...
impl Server {
        ...
        loop {
            ...
            if now - last_zombie_check > self.zombie_check_interval {
                ...
                for session in self
                    .sessions
                    .iter_mut()
                    .filter(|c| now - c.borrow().last_event() > duration)
                {
                ...
                &Query::Applications(ref query_type) => {
                    let answer = match query_type {
                        ...
                        &QueryApplicationType::Domain(ref domain) => {
                            let app_ids = get_application_ids_by_domain(
                                &self.config_state,
                                domain.hostname.clone(),
                                domain.path_begin.clone(),
                            );
                            let answer = app_ids
                                .iter()
                                .map(|ref app_id| self.config_state.application_state(app_id))
                                .collect();
    ...
    pub fn notify_proxys(&mut self, message: ProxyRequest) {
        ...
        match message {
            ProxyRequest {
                order: ProxyRequestData::AddApplication(ref application),
                ..
            } => {
                self.backends
                    .borrow_mut()
            ...
            ProxyRequest {
                ...
                self.backends
                    .borrow_mut()
                    .add_backend(&backend.app_id, new_backend);
            ...
            ProxyRequest {
                ...
                self.backends
                    .borrow_mut()
                    .remove_backend(&backend.app_id, &backend.address);
                ...
                ProxyRequest {
                    ...
                    if activate.proxy == ListenerType::HTTP {
                        ...
                        let listener = self
                            .scm_listeners
                            .as_mut()
                            .and_then(|s| s.get_http(&activate.front))
                ...
                ProxyRequest {
                    ...
                    if activate.proxy == ListenerType::HTTPS {
                        ...
                        let listener = self
                            .scm_listeners
                            .as_mut()
                            .and_then(|s| s.get_https(&activate.front))
                            .map(|fd| unsafe { TcpListener::from_raw_fd(fd) });
                ...
                ProxyRequest {
                    ...
                    if activate.proxy == ListenerType::TCP {
                        ...
                        let listener = self
                            .scm_listeners
                            .as_mut()
                            .and_then(|s| s.get_tcp(&activate.front))
                            .map(|fd| unsafe { TcpListener::from_raw_fd(fd) });
    ...
    pub fn return_listen_sockets(&mut self) {
        ...
        let listeners = Listeners {
            http: http_listeners
                .into_iter()
                .map(|(addr, listener)| (addr, listener.into_raw_fd()))
                .collect(),
            tls: https_listeners
                .into_iter()
                .map(|(addr, listener)| (addr, listener.into_raw_fd()))
    ...
    pub fn create_session_tcp(&mut self, token: ListenToken, socket: TcpStream) -> bool {
        ...
        let index = match self.sessions.vacant_entry() {
            ...
            Some(entry) => {
                ...
                let timeout = self
                    .timer
                    .set_timeout(self.front_timeout.to_std().unwrap(), session_token);
                match self
                    .tcp
                    .create_session(socket, token, &mut self.poll, session_token, timeout)
                {
                    ...
                    Err(AcceptError::TooManySessions) => {
                        error!(
                            "max number of session connection reached, flushing the accept queue"
                        );
                        self.can_accept = false;
                        return false;
                    }
        ...
        match self.sessions.vacant_entry() {
            ...
            Some(entry) => {
                ...
                let timeout = self
                    .timer
                    .set_timeout(self.front_timeout.to_std().unwrap(), session_token);
                match self.http.create_session(
                    socket,
                    token,
                    &mut self.poll,
                    session_token,
        ...
        match self.sessions.vacant_entry() {
            ...
            Some(entry) => {
                ...
                let timeout = self
                    .timer
                    .set_timeout(self.front_timeout.to_std().unwrap(), session_token);
                match self.https.create_session(
                    socket,
                    token,
                    &mut self.poll,
                    session_token,
        ...
        if self.sessions.contains(session_token) {
            ...
            loop {
                ...
                trace!(
                    ...
                    session_token,
                    self.from_session(session_token),
                    events,
                    order
                );
    ...
    pub fn timeout(&mut self, token: Token) {
        ...
        if self.sessions.contains(session_token) {
            let order = self.sessions[session_token].borrow_mut().timeout(
                token,
                &mut self.timer,
                &self.front_timeout,
            );
...
impl HttpsProvider {
    ...
    pub fn create_session(
        ...
        match self {
            &mut HttpsProvider::Rustls(ref mut rustls) => rustls
                .create_session(frontend_sock, token, poll, session_token, timeout)
                .map(|(r, b)| (r as Rc<RefCell<ProxySessionCast>>, b)),
    ...
    pub fn connect_to_backend(
        &mut self,
        poll: &mut Poll,
        proxy_session: Rc<RefCell<ProxySessionCast>>,
        back_token: Token,
