...
use sozu_command::proxy::{
    Application, HttpFront, HttpListener, ProxyEvent, ProxyRequest, ProxyRequestData,
        ...
        match ::idna::domain_to_ascii(&http_front.hostname) {
            Ok(hostname) => {
                ...
                if let Some((_, ref mut fronts)) = self
                    .fronts
                    .domain_lookup_mut(&http_front.hostname.clone().into_bytes())
                {
                ...
                if self
                    .fronts
                    .domain_lookup(&http_front.hostname.clone().into_bytes())
                    .is_none()
                {
                    self.fronts
                        .domain_insert(http_front.hostname.into_bytes(), vec![front3]);
                }
                Ok(())
            }
            Err(_) => Err(format!(
                "Couldn't parse hostname {} to ascii",
                http_front.hostname
            )),
        }
    }
    ...
    fn notify(&mut self, event_loop: &mut Poll, message: ProxyRequest) -> ProxyResponse {
        ...
        match message.order {
            ...
            ProxyRequestData::RemoveListener(remove) => {
                ...
                ProxyResponse {
                    id: message.id,
                    status: ProxyResponseStatus::Error(String::from("unimplemented")),
                    data: None,
                }
            }
        ...
        if let Some(ref listener) = self.listeners.get(&Token(listen_token.0)) {
            ...
            if let Some(c) = Session::new(
                ...
                if let Err(e) = poll.register(
                    ...
                    error!(
                        "error deregistering listen socket({:?}): {:?}",
                        c.front_socket(),
                        e
                    );
                }
...
mod tests {
    ...
    use sozu_command::proxy::{
        Backend, HttpFront, HttpListener, LoadBalancingParams, ProxyRequest, ProxyRequestData,
    };
    ...
    */
    ...
    fn mi() {
        ...
        command.write_message(&ProxyRequest {
            ...
            order: ProxyRequestData::AddHttpFront(front),
        });
        let backend = Backend {
            app_id: String::from("app_1"),
    ...
    fn https_redirect() {
        ...
        command.write_message(&ProxyRequest {
            id: String::from("ID_ABCD"),
            order: ProxyRequestData::AddApplication(application),
        });
        let front = HttpFront {
            app_id: String::from("app_1"),
