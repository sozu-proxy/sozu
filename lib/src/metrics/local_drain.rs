...
use sozu_command::{
    ...
    worker::{AvailableWorkerMetrics, BackendMetrics},
};
...
impl LocalDrain {
    ...
    pub fn query(&mut self, options: &QueryMetricsOptions) -> Result<ResponseContent, String> {
        ...
        match worker_metrics {
            Ok(worker_metrics) => Ok(ResponseContent::WorkerMetrics(worker_metrics)),
    ...
    fn list_all_metric_names(&self) -> ResponseContent {
        ...
        ResponseContent::AvailableWorkerMetrics(AvailableWorkerMetrics {
            proxy_metrics: proxy_metrics_names,
