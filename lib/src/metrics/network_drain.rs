...
impl NetworkDrain {
    pub fn new(prefix: String, socket: UdpSocket, addr: SocketAddr) -> Self {
        NetworkDrain {
            queue: VecDeque::new(),
            prefix,
            remote: LineWriter::with_capacity(2048, MetricSocket { addr, socket }),
            is_writable: true,
            data: BTreeMap::new(),
    ...
    pub fn send_data(&mut self) {
        ...
        if self.is_writable {
            for (ref key, ref mut stored_metric) in self
                .data
                .iter_mut()
                .filter(|&(_, ref value)| now.duration_since(value.last_sent) > secs)
            {
                ...
                .filter(|&(_, ref value)| now.duration_since(value.last_sent) > secs)
            {
                ...
                let res = match stored_metric.data {
                    MetricData::Gauge(value) => {
                        if self.use_tagged_metrics {
                            self.remote.write_fmt(format_args!(
                                "{}.backend.{},origin={},version={},app_id={},backend_id={}:{}|g\n",
                    ...
                    MetricData::Count(value) => {
                        ...
                        let res = if self.use_tagged_metrics {
                            self.remote.write_fmt(format_args!(
                                "{}.app.{},origin={},version={},app_id={},backend_id={}:{}|c\n",
                                self.prefix, key.2, self.origin, VERSION, key.0, key.1, value
                            ))
                        } else {
                            self.remote.write_fmt(format_args!(
              ...
              self.prefix, metric.label, self.origin, VERSION, app_id, backend_id, metric.duration))
                    ...
                    (Some(app_id), None) => {
                        if self.use_tagged_metrics {
                            ...
                            self.remote.write_fmt(format_args!(
                                "{}.{}.app.{}.{}:{}|ms\n",
                                self.prefix, self.origin, app_id, metric.label, metric.duration
                            ))
                        }
                    }
                    (None, None) => {
