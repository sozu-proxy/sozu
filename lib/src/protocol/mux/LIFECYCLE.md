...
- `MuxResult::CloseSession` from any readable/writable path (`mod.rs:2138`,
  `mod.rs:2510`, etc.).
...
3. A `StreamState::Linked(token)` must have a matching entry in
   ...
   `mod.rs:2784-2814` after every `ready()` pass.
...
- Fired when: no traffic observed for `configured_frontend_timeout`
  ...
  until the first `Link` transition (`mod.rs:2569-2571`).
...
- Handling: `Mux::timeout` inspects each stream's state (`mod.rs:2888`) and
...
- Action: for each stream linked to that backend, either send 504, or forcefully
  terminate, or keep draining — see `mod.rs:2989-3044`. The timeout is re-armed
  if the session stays alive (`mod.rs:3119`) to avoid the "immortal zombie"
  state.
...
- once per **outer** `Mux::ready` pass (`mod.rs:2118`), so that the inner loop
  ...
  count and bounds iterations, not wall clock, and `counter` (`mod.rs:2070`)
...
- at the top of `Mux::shutting_down_inner` (`mod.rs:3217`), which runs outside
...
- **Proxy-initiated, idle cancel** — `cancel_timed_out_streams` (§7.2); the
  per-stream `forcefully_terminate_answer` call in the `mod.rs:2943` timeout
...
1. Initiates the double-GOAWAY (`mod.rs:3231`).
...
2. **Backend index consistency.** If
   ...
   `debug_assertions` in `Mux::ready` at `mod.rs:2784-2814`.
...
10. **Single `graceful_goaway` per session outside the final GOAWAY.**
    `Mux::shutting_down_inner` (`mod.rs:3230-3231`) only calls it if
