...
- `ClientPreface` → `ClientSettings` at `h2.rs:2018`.
...
- `Discard` (stream refused) is set in `refuse_stream_and_discard`
  ...
  (`h2.rs:1999` — one of four identical `self.expect_header();` lines, so the
...
- Frontend HUP — detected at `mod.rs:1357`, subject to
  `delay_close_for_frontend_flush` (`mod.rs`) to avoid truncating TLS.
- `MuxResult::CloseSession` from any readable/writable path (`mod.rs:1422`,
  `mod.rs:1749`, etc.).
- A loop-iteration budget overrun (`MAX_LOOP_ITERATIONS = 10_000`, `mod.rs`,
  check at `mod.rs:1782`).
...
- The pool buffers have been cleared (`mod.rs:682-685`).
- Metrics have been reset (`mod.rs:687`).
...
- Mutated by:
  - push — `create_stream` when no `Recycle` slot is available (`mod.rs:639-640`).
...
3. A `StreamState::Linked(token)` must have a matching entry in
   ...
   `mod.rs:1962-1992` after every `ready()` pass.
...
- `read_buffer` — indexes `context.streams[global_stream_id]` on behalf of
  ...
  `H2StreamId::Other` arm does the dereference at `h2.rs:1056`. `readable`
...
recycled slot to reuse and then crosses the shrink ratio (`mod.rs:693-694`).
...
- `write_streams` after end-of-stream — `h2.rs:2545`/`h2.rs:3121`.
- `prune_inactive_streams_while_closing` — `h2.rs:3331`.
- `handle_window_update_frame` zero-increment path — `h2.rs:6129`.
- `cancel_timed_out_streams` slow-multiplex guard — `h2.rs:4299`.
- `handle_continuation_header_state` CONTINUATION oversize — `h2.rs:1728`.
- `handle_rst_stream_frame` peer RST — `h2.rs:5833`.
- `handle_goaway_frame` retry loop — `h2.rs:6079`.
- `end_stream` client-side retirement — `h2.rs:6613`.
...
- `close` backend-stream teardown — `h2.rs:6479-6481` (does not remove, only
...
by a ratio threshold so we don't thrash on every request (`mod.rs:693-694`):
...
(`h2.rs:309`). In short: if more than `2×active` slots are held, trim trailing
`Recycle` entries.
...
- Fired when: no traffic observed for `configured_frontend_timeout`
  (`mod.rs:735`) while any stream is live, or the shorter `request_timeout`
  until the first `Link` transition (`mod.rs:1809-1811`).
- Reset: on meaningful activity — HEADERS for an existing stream, DATA bytes —
  see `h2.rs:5400` and the `arm_timeout()` call in the DATA-payload arm of
  ...
  peer cannot pin the session with keepalive noise (`h2.rs:1840-1846`).
- Handling: `Mux::timeout` inspects each stream's state (`mod.rs:2054`) and
...
- **Outbound-flow-control-stall guard** — `ConnectionH2.stream_fc_stalled_since:
  ...
  **bidirectional**: the buffered data is the **response** on a `Position::Server`
  ...
  resume path (the `resume_bytes > 0` block of `ConnectionH2::write_streams`,
  `h2.rs`) — therefore never reaches the floor, so the deadline
...
- Action: for each stream linked to that backend, either send 504, or forcefully
  terminate, or keep draining — see `mod.rs:2149-2203`. The timeout is re-armed
  if the session stays alive (`mod.rs:2276`) to avoid the "immortal zombie"
  state.
...
2. Then that same call optionally fires `goaway(SettingsTimeout)` if the
   SETTINGS ACK is overdue (`h2.rs:1828-1838`) — before any read is offered.
...
6. Loop budget (`MAX_LOOP_ITERATIONS = 10_000`, `mod.rs`) is a hard backstop
   at `mod.rs:1782`. `counter` is declared at `mod.rs:1355`, above BOTH loops, so
...
`Context.now` (`mod.rs:542`) at three points:

- once per **outer** `Mux::ready` pass (`mod.rs:1403`), so that the inner loop
...
- at the top of `Mux::shutting_down` (`mod.rs:2374`), which runs outside
...
The H2 core reads `ConnectionH2.now` (`h2.rs:877`), a mirror assigned from
...
- An **arm** site runs at an arbitrary depth into its pass — the liveness
  ...
  convert it back) and at `h2.rs:5431` (HEADERS), the
  outbound-byte refreshes at `h2.rs:2498` / `h2.rs:2938-2940`, the
...
- An **eval** site runs near the top of a pass: `cancel_timed_out_streams` is
  ...
  (`h2.rs:1997` in `poll_read_target`, `h2.rs:3772` in `flush_pending_control_frames`)
  is step 2.
...
RFC 9113 §5.1.2 back-pressure window (`h2.rs:4476`) are the one asymmetric case,
and they
**fail closed**: `now` is
...
1. **Initial GOAWAY** — send GOAWAY with `last_stream_id = 0x7FFFFFFF`
   ...
   `mod.rs:2381`. Draining flag set.
2. **Final GOAWAY** — on the second invocation (draining already true), call
   `goaway(NoError)` (`h2.rs:4722`) with the actual `highest_peer_stream_id`,
   remove `READABLE` interest (`h2.rs:4678`), transition to `H2State::GoAway`.
   Caller is `finalize_write` when all streams drain (`h2.rs:3366`).
...
- **Proxy-initiated, idle cancel** — `cancel_timed_out_streams` (§7.2); the
  per-stream `forcefully_terminate_answer` call in the `mod.rs:2103` timeout
...
- DATA-on-closed-stream (`h2.rs:1612` — `H2Error::StreamClosed`).
...
1. Initiates the double-GOAWAY (`mod.rs:2381`).
...
- **Client** position (i.e. the backend's view) — `h2.rs:6575-6624`. Sends
...
- **Server** position — the `Position::Server` arm of `ConnectionH2::end_stream`
  (`h2.rs:6625-6732`; the range start is one of eight identical
...
2. **Backend index consistency.** If
   ...
   `debug_assertions` in `Mux::ready` at `mod.rs:1962-1992`.
...
8. **No new streams during drain.** `create_stream` and `start_stream` both
   short-circuit when `self.drain.draining()` (`h2.rs:4929-4936`,
   `h2.rs:6746-6753`).
...
10. **Single `graceful_goaway` per session outside the final GOAWAY.**
    `Mux::shutting_down` (`mod.rs:2380-2381`) only calls it if
...
20. **Only `Mux` samples the clock.** No code under `ConnectionH2` calls
    ...
    Every hit must be inside `#[cfg(test)] mod tests` (`h2.rs:6814` onward) or
...
26. **RFC 9218 §4 incremental leadership rotates inside the bucket that
    ...
    **The bound covers ONE bucket per connection, and the limitation is
    ...
    leads. That is positional starvation, and a stalled flush at
...
27. **A close under TLS backpressure asks TWO questions, not one, and asks
    ...
    records — the `H2State::GoAway` arm of `ConnectionH2::writable`, the
    ...
    **A fourth site shares the shape without deciding a close.**
    ...
    query / flush / query triple, and its answer is
    `h2_close::finalize_action`. It is a sibling `FinalizeAction` and not four
    ...
    `match` `writable` already writes over it. Its middle flush is also
...
- **Doubled metrics / gauge drift** — check §9 item 14 and the backend-stream
  accounting paths in `mod.rs:1313-1314` and `mod.rs:1647-1648`.
- **Truncated response / TLS decode error** — check `has_pending_write`
  ...
  (`h2.rs:6369-6467`; the range start is one of eight identical
...
The TLS-drain warning at `ConnectionH2::close` (`h2.rs:6406-6466`, `Position::Server`
...
`goaway()` (`h2.rs:4660-4664`); both rules demote benign paths and keep
