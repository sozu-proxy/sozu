...
- `MuxResult::CloseSession` from any readable/writable path (`mod.rs:1511`,
  `mod.rs:1838`, etc.).
- A loop-iteration budget overrun (`MAX_LOOP_ITERATIONS = 10_000`, `mod.rs`,
  check at `mod.rs:1871`).
...
- **Backend attach.** `Router::connect` (called from `mod.rs:1902` during the
...
- The pool buffers have been cleared (`mod.rs:770-773`).
...
- Mutated by:
  - push — `create_stream` when no `Recycle` slot is available (`mod.rs:787`).
...
3. A `StreamState::Linked(token)` must have a matching entry in
   ...
   `mod.rs:2053-2083` after every `ready()` pass.
...
recycled slot to reuse and then crosses the shrink ratio (`mod.rs:781-782`).
...
- `poll_write_target` after end-of-stream — the `H2WritePhase::Resume` retirement
  at `h2.rs:2524` and `H2WritePhase::End`'s deferred `completed_streams` loop at
  `h2.rs:2922`.
- `ConnectionH2::prune_inactive_streams_while_closing` (`h2.rs`).
- `handle_window_update_frame` zero-increment path — `h2.rs:6095`.
...
- `close` backend-stream teardown — `h2.rs:6453-6455` (does not remove, only
...
by a ratio threshold so we don't thrash on every request (`mod.rs:781-782`):
...
- Fired when: no traffic observed for `configured_frontend_timeout`
  (`mod.rs:823`) while any stream is live, or the shorter `request_timeout`
  until the first `Link` transition (`mod.rs:1897-1899`).
...
- Handling: `Mux::timeout` inspects each stream's state (`mod.rs:2145`) and
...
- Action: for each stream linked to that backend, either send 504, or forcefully
  terminate, or keep draining — see `mod.rs:2246-2301`. The timeout is re-armed
  if the session stays alive (`mod.rs:2376`) to avoid the "immortal zombie"
  state.
...
6. Loop budget (`MAX_LOOP_ITERATIONS = 10_000`, `mod.rs`) is a hard backstop
   at `mod.rs:1871`. `counter` is declared at `mod.rs:1443`, above BOTH loops, so
...
`Context.now` (`mod.rs:546`) at three points:

- once per **outer** `Mux::ready` pass (`mod.rs:1491`), so that the inner loop
  ...
  count and bounds iterations, not wall clock, and `counter` (`mod.rs:1443`)
  sits above both loops, so one `ready()` call can spend the whole budget
  under a single snapshot;
- at the top of `Mux::timeout_inner` (`mod.rs:2092`);
- at the top of `Mux::shutting_down_inner` (`mod.rs:2474`), which runs outside
...
- An **arm** site runs at an arbitrary depth into its pass — the liveness
  ...
  outbound-byte refreshes at `h2.rs:2481` (`H2WritePhase::Resume`) and
  `h2.rs:2760-2762` (`H2WritePhase::Flush`), the `FcStallAction::Arm` branch of
...
- An **eval** site runs near the top of a pass: `cancel_timed_out_streams` is
  ...
  (`h2.rs:1973` in `poll_read_target`, `h2.rs:3517` in `flush_pending_control_frames`)
  is step 2.
...
RFC 9113 §5.1.2 back-pressure window (`h2.rs:4439`) are the one asymmetric case,
and they
**fail closed**: `now` is
...
1. **Initial GOAWAY** — send GOAWAY with `last_stream_id = 0x7FFFFFFF`
   ...
   `mod.rs:2482`. Draining flag set.
2. **Final GOAWAY** — on the second invocation (draining already true), call
   `goaway(NoError)` (`h2.rs:4685`) with the actual `highest_peer_stream_id`,
   remove `READABLE` interest (`h2.rs:4641`), transition to `H2State::GoAway`.
...
- **Proxy-initiated, idle cancel** — `cancel_timed_out_streams` (§7.2); the
  per-stream `forcefully_terminate_answer` call in the `mod.rs:2200` timeout
...
1. Initiates the double-GOAWAY (`mod.rs:2482`).
...
- **Client** position (i.e. the backend's view) — `h2.rs:6549-6598`. Sends
...
- **Server** position — the `Position::Server` arm of `ConnectionH2::end_stream`
  (`h2.rs:6599-6706`; the range start is one of eight identical
...
2. **Backend index consistency.** If
   ...
   `debug_assertions` in `Mux::ready` at `mod.rs:2053-2083`.
...
10. **Single `graceful_goaway` per session outside the final GOAWAY.**
    `Mux::shutting_down_inner` (`mod.rs:2481-2482`) only calls it if
...
- **Doubled metrics / gauge drift** — check §9 item 14 and the backend-stream
  accounting paths in `mod.rs:1401-1402` and `mod.rs:1736-1737`.
- **Truncated response / TLS decode error** — check `has_pending_write`
  ...
  (`h2.rs:6343-6441`; the range start is one of eight identical
...
The TLS-drain warning at `ConnectionH2::close` (`h2.rs:6380-6440`, `Position::Server`
...
`goaway()` (`h2.rs:4623-4627`); both rules demote benign paths and keep
