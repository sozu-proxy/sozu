...
- `ClientPreface` → `ClientSettings` at `h2.rs:2318`.
...
- **Backend attach.** Two call sites, on the two paths a stream can reach a
  ...
  One read is left, and it is not a read: the per-(cluster, source-IP) limit
  gate calls `SessionManager::cluster_ip_at_limit` **and**
  ...
  and read again by `lib/src/tcp.rs`'s own gate). It cannot become part of a
  read-only view, so it becomes a returned step in its own right. Backend
  selection likewise still reaches the handle, from `Mux::dial_backend`
  through `Router::backend_from_request`; giving it a borrowed view of backend
  load state is Question 12's second part.
...
- The pool buffers have been cleared (`mod.rs:1018-1021`).
...
- Mutated by:
  - push — `create_stream` when no `Recycle` slot is available (`mod.rs:1035`).
...
3. A `StreamState::Linked(token)` must have a matching entry in
   ...
   `mod.rs:2672-2702` after every `ready()` pass.
...
- `read_buffer` — indexes `context.streams[global_stream_id]` on behalf of
  ...
  `H2StreamId::Other` arm does the dereference at `h2.rs:1114`. `readable`
...
recycled slot to reuse and then crosses the shrink ratio (`mod.rs:1029-1030`).
...
- `poll_write_target` after end-of-stream — the `H2WritePhase::Resume` retirement
  at `h2.rs:2703` and `H2WritePhase::End`'s deferred `completed_streams` loop at
  `h2.rs:3101`.
- `ConnectionH2::prune_inactive_streams_while_closing` (`h2.rs`).
- `handle_window_update_frame` zero-increment path — `h2.rs:6371`.
...
- `close` backend-stream teardown — `h2.rs:7394-7396` (does not remove, only
...
by a ratio threshold so we don't thrash on every request (`mod.rs:1029-1030`):
...
- Handling: `Mux::timeout` inspects each stream's state (`mod.rs:2776`) and
...
- Action: for each stream linked to that backend, either send 504, or forcefully
  terminate, or keep draining — see `mod.rs:2877-2932`. The timeout is re-armed
  if the session stays alive (`mod.rs:3007`) to avoid the "immortal zombie"
  state.
...
2. Then that same call optionally fires `goaway(SettingsTimeout)` if the
   SETTINGS ACK is overdue (`h2.rs:2125-2135`) — before any read is offered.
...
- at the top of `Mux::timeout_inner` (`mod.rs:2711`);
- at the top of `Mux::shutting_down_inner` (`mod.rs:3105`), which runs outside
...
- An **arm** site runs at an arbitrary depth into its pass — the liveness
  ...
  convert it back) and at `h2.rs:5710` (HEADERS), the
  ...
  `h2.rs:2939-2941` (`H2WritePhase::Flush`), the `FcStallAction::Arm` branch of
...
- An **eval** site runs near the top of a pass: `cancel_timed_out_streams` is
  ...
  (`h2.rs:2125` in `poll_read_target`, `h2.rs:3707` in `flush_pending_control_frames`)
  is step 2.
...
1. **Initial GOAWAY** — send GOAWAY with `last_stream_id = 0x7FFFFFFF`
   ...
   `mod.rs:3119`. Draining flag set.
2. **Final GOAWAY** — on the second invocation (draining already true), call
   `goaway(NoError)` (`h2.rs:4950`) with the actual `highest_peer_stream_id`,
   remove `READABLE` interest (`h2.rs:4906`), transition to `H2State::GoAway`.
...
- **Proxy-initiated, idle cancel** — `cancel_timed_out_streams` (§7.2); the
  per-stream `forcefully_terminate_answer` call in the `mod.rs:2831` timeout
...
- DATA-on-closed-stream (`h2.rs:1909` — `H2Error::StreamClosed`).
...
1. Initiates the double-GOAWAY (`mod.rs:3119`).
...
- **Client** position (i.e. the backend's view) — `h2.rs:6775-6824`. Sends
...
- **Server** position — the `Position::Server` arm of `ConnectionH2::end_stream`
  (`h2.rs:6825-6932`; the range start is one of eight identical
...
2. **Backend index consistency.** If
   ...
   `debug_assertions` in `Mux::ready` at `mod.rs:2672-2702`.
...
10. **Single `graceful_goaway` per session outside the final GOAWAY.**
    `Mux::shutting_down_inner` (`mod.rs:3118-3119`) only calls it if
...
`goaway()` (`h2.rs:4888-4892`); both rules demote benign paths and keep
