...
- `ClientPreface` → `ClientSettings` at `h2.rs:2228`.
...
- `MuxResult::CloseSession` from any readable/writable path (`mod.rs:1553`,
  `mod.rs:1880`, etc.).
- A loop-iteration budget overrun (`MAX_LOOP_ITERATIONS = 10_000`, `mod.rs`,
  check at `mod.rs:1913`).
...
- **Backend attach.** `Router::connect` (called from `mod.rs:1944` during the
...
3. A `StreamState::Linked(token)` must have a matching entry in
   ...
   `mod.rs:2095-2125` after every `ready()` pass.
...
- `read_buffer` — indexes `context.streams[global_stream_id]` on behalf of
  ...
  `H2StreamId::Other` arm does the dereference at `h2.rs:1037`. `readable`
...
- `poll_write_target` after end-of-stream — the `H2WritePhase::Resume` retirement
  at `h2.rs:2586` and `H2WritePhase::End`'s deferred `completed_streams` loop at
  `h2.rs:2984`.
- `ConnectionH2::prune_inactive_streams_while_closing` (`h2.rs`).
- `handle_window_update_frame` zero-increment path — `h2.rs:6187`.
...
- `close` backend-stream teardown — `h2.rs:6545-6547` (does not remove, only
...
- Fired when: no traffic observed for `configured_frontend_timeout`
  ...
  until the first `Link` transition (`mod.rs:1939-1941`).
...
- Handling: `Mux::timeout` inspects each stream's state (`mod.rs:2199`) and
...
- Action: for each stream linked to that backend, either send 504, or forcefully
  terminate, or keep draining — see `mod.rs:2300-2355`. The timeout is re-armed
  if the session stays alive (`mod.rs:2430`) to avoid the "immortal zombie"
  state.
...
2. Then that same call optionally fires `goaway(SettingsTimeout)` if the
   SETTINGS ACK is overdue (`h2.rs:2035-2045`) — before any read is offered.
...
6. Loop budget (`MAX_LOOP_ITERATIONS = 10_000`, `mod.rs`) is a hard backstop
   at `mod.rs:1913`. `counter` is declared at `mod.rs:1485`, above BOTH loops, so
...
- once per **outer** `Mux::ready` pass (`mod.rs:1533`), so that the inner loop
  ...
  count and bounds iterations, not wall clock, and `counter` (`mod.rs:1485`)
  sits above both loops, so one `ready()` call can spend the whole budget
  under a single snapshot;
- at the top of `Mux::timeout_inner` (`mod.rs:2134`);
- at the top of `Mux::shutting_down_inner` (`mod.rs:2528`), which runs outside
...
- An **arm** site runs at an arbitrary depth into its pass — the liveness
  ...
  convert it back) and at `h2.rs:5523` (HEADERS), the
  ...
  `h2.rs:2822-2824` (`H2WritePhase::Flush`), the `FcStallAction::Arm` branch of
...
- An **eval** site runs near the top of a pass: `cancel_timed_out_streams` is
  ...
  (`h2.rs:2035` in `poll_read_target`, `h2.rs:3580` in `flush_pending_control_frames`)
  is step 2.
...
RFC 9113 §5.1.2 back-pressure window (`h2.rs:4531`) are the one asymmetric case,
and they
**fail closed**: `now` is
...
1. **Initial GOAWAY** — send GOAWAY with `last_stream_id = 0x7FFFFFFF`
   ...
   `mod.rs:2542`. Draining flag set.
2. **Final GOAWAY** — on the second invocation (draining already true), call
   `goaway(NoError)` (`h2.rs:4777`) with the actual `highest_peer_stream_id`,
   remove `READABLE` interest (`h2.rs:4733`), transition to `H2State::GoAway`.
...
- **Proxy-initiated, idle cancel** — `cancel_timed_out_streams` (§7.2); the
  per-stream `forcefully_terminate_answer` call in the `mod.rs:2254` timeout
...
- DATA-on-closed-stream (`h2.rs:1819` — `H2Error::StreamClosed`).
...
1. Initiates the double-GOAWAY (`mod.rs:2542`).
...
- **Client** position (i.e. the backend's view) — `h2.rs:6641-6690`. Sends
...
- **Server** position — the `Position::Server` arm of `ConnectionH2::end_stream`
  (`h2.rs:6691-6798`; the range start is one of eight identical
...
2. **Backend index consistency.** If
   ...
   `debug_assertions` in `Mux::ready` at `mod.rs:2095-2125`.
...
10. **Single `graceful_goaway` per session outside the final GOAWAY.**
    `Mux::shutting_down_inner` (`mod.rs:2541-2542`) only calls it if
...
- **Doubled metrics / gauge drift** — check §9 item 14 and the backend-stream
  accounting paths in `mod.rs:1443-1444` and `mod.rs:1778-1779`.
- **Truncated response / TLS decode error** — check `has_pending_write`
  ...
  (`h2.rs:6435-6533`; the range start is one of eight identical
...
The TLS-drain warning at `ConnectionH2::close` (`h2.rs:6472-6532`, `Position::Server`
...
`goaway()` (`h2.rs:4715-4719`); both rules demote benign paths and keep
