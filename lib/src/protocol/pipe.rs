...
use {Readiness, SessionMetrics, SessionResult};
...
impl<Front: SocketHandler> Pipe<Front> {
    pub fn new(
        ...
        let session = Pipe {
            ...
            front_readiness: Readiness {
                interest: UnixReady::from(Ready::readable() | Ready::writable())
                    | UnixReady::hup()
                    | UnixReady::error(),
    ...
    pub fn back_writable(&mut self, metrics: &mut SessionMetrics) -> SessionResult {
        ...
        match socket_res {
            SocketResult::Error | SocketResult::Closed => {
                error!(
                    "{}\tback socket write error, closing connection",
                    self.log_ctx
                );
                metrics.service_stop();
                incr!("pipe.errors");
    ...
    pub fn back_readable(&mut self, metrics: &mut SessionMetrics) -> SessionResult {
        ...
        if let Some(ref mut backend) = self.backend {
            ...
            match r {
                SocketResult::Error => {
                    error!(
                        "{}\tback socket read error, closing connection",
                        self.log_ctx
                    );
                    metrics.service_stop();
                    incr!("pipe.errors");
