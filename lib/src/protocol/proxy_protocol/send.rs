...
impl<Front: SocketHandler + Read> SendProxyProtocol<Front> {
    ...
    pub fn set_back_connected(&mut self, status: BackendConnectionStatus) {
        if status == BackendConnectionStatus::Connected {
            self.back_readiness
                .interest
                .insert(UnixReady::from(Ready::writable()));
        }
    }
    ...
    pub fn into_pipe(
        ...
        pipe.front_readiness
            .interest
            .insert(UnixReady::from(Ready::readable()));
        pipe.back_readiness
            .interest
            .insert(UnixReady::from(Ready::readable()));
...
mod send_test {
    ...
    fn start_middleware(addr_client: SocketAddr, addr_backend: SocketAddr, barrier: Arc<Barrier>) {
        let listener =
            TcpListener::bind(&addr_client).expect("could not accept session connection");
