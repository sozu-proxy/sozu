...
use nom::{is_alphanumeric, is_space, AsChar};
...
named!(
    digit_complete,
    take_while1_complete!(|item: u8| item.is_dec_digit())
);
...
pub fn comma_separated_header_values(input: &[u8]) -> Option<Vec<&[u8]>> {
    let res: IResult<&[u8], Vec<&[u8]>> = separated_list!(
        input,
        delimited!(opt!(complete!(sp)), complete!(char!(',')), opt!(sp)),
        single_header_value
    );
    if let Ok((_, o)) = res {
        Some(o)
...
impl Chunk {
    ...
    pub fn parse_one(&self, buf: &[u8]) -> (usize, Chunk) {
        match *self {
            ...
            Chunk::CopyingLastHeader => match crlf(buf) {
                Ok((i, _)) => (buf.offset(i), Chunk::Ended),
...
impl<'a> Header<'a> {
    ...
    pub fn should_delete(&self, conn: &Connection, sticky_name: &str) -> bool {
        ...
        if compare_no_case(&self.name, b"connection") {
            ...
            (compare_no_case(&self.name, b"connection")
    ...
    pub fn remove_sticky_cookie_in_request(
        &self,
        buf: &[u8],
        offset: usize,
        sticky_name: &str,
        ...
        if let Some(cookies) = parse_request_cookies(self.value) {
                ...
                .position(|cookie| &cookie.name[..] == sticky_name.as_bytes())
            {
                ...
                moves.push(BufferMove::Advance(
                    header_length + length_until_value.len(),
                ));
                        ...
                        Some(cookie) => {
                            ...
                            if current_cookie == sozu_balance_position {
                                ...
                                if (current_cookie + 1) == sozu_balance_position {
                                    ...
                                    let semicolon = 1;
                                    moves.push(BufferMove::Advance(
                                        cookie_length - spaces - semicolon,
                                    ));
    ...
    match header.value() {
        ...
        HeaderValue::Encoding(TransferEncodingValue::Chunked) => match state {
            ResponseState::HasStatusLine(rl, conn) => {
                ResponseState::HasLength(rl, conn, LengthInformation::Chunked)
            }
            s => s.into_error(),
        },
        // FIXME: for now, we don't remember if we cancel indications from a previous Connection Header
...
pub fn parse_response(
    ...
    match state {
        ...
        ResponseState::HasStatusLine(sl, conn) => match message_header(buf) {
            Ok((i, header)) => {
                let mv = if header.should_delete(&conn, sticky_name) {
                    BufferMove::Delete(buf.offset(i))
                } else {
            ...
            Err(_) => match crlf(buf) {
                ...
                res => {
                    error!(
                        "PARSER\tHasResponseLine could not parse header for input:\n{}\n",
                        buf.to_hex(16)
                    );
        ...
        ResponseState::HasLength(sl, conn, length) => match message_header(buf) {
            Ok((i, header)) => {
                let mv = if header.should_delete(&conn, sticky_name) {
                    BufferMove::Delete(buf.offset(i))
                } else {
...
pub fn parse_response_until_stop(
    ...
    loop {
        ...
        let (mv, new_state) =
            parse_response(current_state, buf.unparsed_data(), is_head, sticky_name);
        //trace!("PARSER\t{}\tinput:\n{}\nmv: {:?}, new state: {:?}\n", request_id, buf.unparsed_data().to_hex(16), mv, new_state);
        //trace!("PARSER\t{}\tmv: {:?}, new state: {:?}\n", request_id, mv, new_state);
        current_state = new_state;
    ...
    */
    ...
    fn parse_state_host_in_url_test() {
        ...
        assert_eq!(
            buf.output_queue,
            vec!(
                OutputElement::Slice(49),
                OutputElement::Slice(25),
                OutputElement::Slice(13),
    ...
    fn parse_state_host_in_url_conflict_test() {
        ...
        assert_eq!(
            buf.output_queue,
            vec!(OutputElement::Slice(49), OutputElement::Slice(16))
        );
    ...
    fn parse_state_content_length_partial() {
        ...
        buf.slice_output(26);
        println!(
            "unparsed data after consume(26):\n{}",
            buf.unparsed_data().to_hex(16)
        );
        println!("buffer output: {:?}", buf.output_queue);
        ...
        assert_eq!(
            ...
            (
                RequestState::Error(
                    ...
                    Some(Connection::new()),
                    Some(String::from("localhost:8888")),
                    Some(LengthInformation::Length(120)),
                    None
                ),
                None
            )
    ...
    fn parse_request_add_header_test() {
        ...
        let result = parse_request_until_stop(
            initial,
            None,
            &mut buf,
            "Sozu-Id: 123456789\r\n",
    ...
    fn parse_response_and_chunks_partial_test() {
        ...
        let result =
            parse_response_until_stop(initial, None, &mut buf, false, "", "SOZUBALANCEID", None);
        println!("result({}): {:?}", line!(), result);
        ...
        let result = parse_response_until_stop(
            result.0,
            result.1,
            &mut buf,
            false,
    ...
    fn parse_incomplete_chunk_header_test() {
        ...
        assert_eq!(
            buf.output_queue,
            vec!(OutputElement::Insert(vec!()), OutputElement::Slice(2))
        );
    ...
    fn parse_state_head_with_content_length_test() {
        ...
        //let result = parse_request(initial, input);
        let result =
            parse_response_until_stop(initial, None, &mut buf, is_head, "", "SOZUBALANCEID", None);
        println!("result: {:?}", result);
        println!("input length: {}", input.len());
        println!("buffer input: {:?}", buf.input_queue);
        println!("buffer output: {:?}", buf.output_queue);
