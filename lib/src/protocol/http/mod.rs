...
use self::parser::{
    parse_request_until_stop, parse_response_until_stop, Chunk, Continue, RRequestLine,
    RStatusLine, RequestState, ResponseState,
};
...
impl<Front: SocketHandler> Http<Front> {
    ...
    pub fn reset(&mut self) {
        ...
        self.added_req_header =
            self.added_request_header(self.public_address, self.session_address);
    ...
    pub fn reset_log_context(&mut self) {
        self.log_ctx = format!(
            "{} {}\t",
            self.request_id,
            self.app_id.as_ref().unwrap_or(&String::from("unknown"))
        );
    }
    ...
    pub fn added_request_header(
        ...
        let peer = client_address
            .or_else(|| self.front_socket().peer_addr().ok())
            .map(|addr| (addr.ip(), addr.port()));
        let front = public_address
            .or_else(|| self.front_socket().local_addr().ok())
            .map(|addr| addr.ip());
        let front_port = public_address
            .or_else(|| self.front_socket().local_addr().ok())
            .map(|addr| addr.port());
        if let (Some((peer_ip, peer_port)), Some(front), Some(front_port)) =
            (peer, front, front_port)
        {
    ...
    pub fn log_request_success(&self, metrics: &SessionMetrics) {
        ...
        let request_line = self
            .get_request_line()
            .map(|line| format!("{} {}", line.method, line.uri))
            .unwrap_or_else(|| String::from("-"));
        let status_line = self
            .get_response_status()
            .map(|line| format!("{} {}", line.status, line.reason))
            .unwrap_or_else(|| String::from("-"));
        ...
        if self.front_buf.as_ref().unwrap().buffer.available_space() == 0 {
            if self.backend_token == None {
                ...
                self.set_answer(
                    ...
                    Rc::new(Vec::from(answer_413.as_bytes())),
                );
    ...
    pub fn writable(&mut self, metrics: &mut SessionMetrics) -> SessionResult {
            ...
            && self.back_buf.as_ref().unwrap().output_data_size() > 0
        {
            ...
            let (current_sz, current_res) = self
                .frontend
