...
use sozu_command::config::{LoadBalancingAlgorithms, ProxyProtocolConfig};
        ...
        if let Some(backend_id) = self.metrics.backend_id.as_ref() {
            if let Some(backend_response_time) = self.metrics.backend_response_time() {
                record_backend_metrics!(
                    app_id,
                    backend_id,
                    backend_response_time.num_milliseconds(),
    ...
    pub fn upgrade(&mut self) -> UpgradeResult {
        ...
        if let Some(State::SendProxyProtocol(pp)) = protocol {
            if self.back_buf.is_some() && self.front_buf.is_some() {
                self.protocol = Some(State::Pipe(pp.into_pipe(
            ...
            if self.back_buf.is_some() {
                self.protocol = Some(State::Pipe(pp.into_pipe(self.back_buf.take().unwrap())));
                gauge_add!("protocol.proxy.relay", -1);
                gauge_add!("protocol.tcp", 1);
                UpgradeResult::Continue
            } else {
...
impl ProxyConfiguration<Session> for Proxy {
    ...
    fn notify(&mut self, event_loop: &mut Poll, message: ProxyRequest) -> ProxyResponse {
        match message.order {
            ...
            ProxyRequestData::RemoveApplication(_) => ProxyResponse {
                id: message.id,
                status: ProxyResponseStatus::Ok,
                data: None,
            },
    ...
    fn listen_port_state(&self, port: &u16) -> ListenPortState {
        match self
            .listeners
            .values()
            .find(|listener| &listener.address.port() == port)
        {
...
mod tests {
    ...
    use sozu_command::proxy::{self, LoadBalancingParams, TcpFront};
    ...
    pub fn start_proxy() -> Channel<ProxyRequest, ProxyResponse> {
        ...
        thread::spawn(move || {
...

            {
                ...
                let entry = sessions
                    .vacant_entry()
                    .expect("session list should have enough room at startup");
                let _ = configuration.add_listener(
                    listener_config,
                    pool.clone(),
                    Token(entry.index().0),
                );
        ...
        command.set_blocking(true);
        {
            ...
            command.write_message(&ProxyRequest {
                ...
                order: ProxyRequestData::AddTcpFront(front),
            });
            command.write_message(&ProxyRequest {
