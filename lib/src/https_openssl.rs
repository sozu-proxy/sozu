...
use openssl::ssl::{
    self, NameType, SniError, Ssl, SslAlert, SslContext, SslContextBuilder, SslMethod, SslOptions,
    SslRef, SslStream,
};
...
use sozu_command::proxy::{
    Application, CertFingerprint, CertificateAndKey, HttpFront, HttpsListener, ProxyEvent,
    ProxyRequest, ProxyRequestData, ProxyResponse, ProxyResponseData, ProxyResponseStatus, Query,
    QueryAnswer, QueryAnswerCertificate, QueryCertificateType, TlsVersion,
};
use sozu_command::scm_socket::ScmSocket;

use backends::BackendMap;
use http::{CustomAnswers, DefaultAnswers};
use pool::Pool;
use protocol::http::parser::{hostname_and_port, RRequestLine, RequestState};
...
use server::{
    push_event, ListenPortState, ListenSession, ListenToken, ProxyChannel, Server, SessionToken,
    CONN_RETRIES,
};
...
use util::UnwrapLog;
use {
    AcceptError, AppId, Backend, BackendConnectAction, BackendConnectionStatus, CloseResult,
...
impl Session {
    pub fn new(
        ...
        session.front_readiness().interest =
            UnixReady::from(Ready::readable()) | UnixReady::hup() | UnixReady::error();
    ...
    fn front_hup(&mut self) -> SessionResult {
        self.http()
            .map(|h| h.front_hup())
            .unwrap_or(SessionResult::CloseSession)
    }
...
impl ProxySession for Session {
    ...
    fn close_backend(&mut self, _: Token, poll: &mut Poll) {
        ...
        if back_connected != BackendConnectionStatus::NotConnected {
            self.back_readiness()
                .map(|r| r.event = UnixReady::from(Ready::empty()));
    ...
    fn process_events(&mut self, token: Token, events: Ready) {
        trace!(
            "token {:?} got event {}",
            token,
            super::unix_ready_to_string(UnixReady::from(events))
        );
        ...
        if self.frontend_token == token {
            ...
            self.back_readiness()
                .map(|r| r.event = r.event | UnixReady::from(events));
        }
    }
    ...
    fn ready(&mut self) -> SessionResult {
        ...
        while counter < max_loop_iterations {
            ...
            let back_interest = self
                ...
                .unwrap_or(UnixReady::from(Ready::empty()));
            ...
            if front_interest == UnixReady::from(Ready::empty())
            ...
            if back_interest.is_hup() {
                ...
                match order {
                    ...
                    _ => {
                        self.back_readiness()
                            .map(|r| r.event.remove(UnixReady::hup()));
                        return order;
                    }
                };
            }
            ...
            if front_interest.is_error() || back_interest.is_error() {
                ...
                self.back_readiness()
                    .map(|r| r.interest = UnixReady::from(Ready::empty()));
...
impl Listener {
    ...
    pub fn activate(
        ...
        let listener = tcp_listener.or_else(|| {
            server_bind(&self.config.front)
                .map_err(|e| {
    ...
    pub fn add_https_front(&mut self, tls_front: HttpFront) -> bool {
        ...
        if let Some((_, ref mut fronts)) = self
            .fronts
            .domain_lookup_mut(&tls_front.hostname.clone().into_bytes())
        {
        ...
        if self
            .fronts
            .domain_lookup(&tls_front.hostname.clone().into_bytes())
            .is_none()
        {
            self.fronts
                .domain_insert(tls_front.hostname.into_bytes(), vec![app]);
        }
    ...
    pub fn add_certificate(&mut self, certificate_and_key: CertificateAndKey) -> bool {
        ...
        if let (Ok(cert), Ok(key)) = (
            ...
            //FIXME
            let fingerprint = CertFingerprint(unwrap_msg!(cert
    ...
    pub fn frontend_from_request(&self, host: &str, uri: &str) -> Option<&TlsApp> {
        ...
        if let Some((_, http_fronts)) = self.fronts.domain_lookup(host.as_bytes()) {
            let matching_fronts = http_fronts
                .iter()
                .filter(|f| uri.starts_with(&f.path_begin)); // ToDo match on uri
            let mut front = None;
...
impl Proxy {
    ...
    pub fn add_application(&mut self, mut application: Application) {
        if let Some(answer_503) = application.answer_503.take() {
            self.custom_answers
                ...
                .and_modify(|c| {
                    c.ServiceUnavailable = Some(Rc::new(answer_503.clone().into_bytes()))
                })
                .or_insert(CustomAnswers {
                    ServiceUnavailable: Some(Rc::new(answer_503.into_bytes())),
                });
        }

        self.applications
            .insert(application.app_id.clone(), application);
    }
    ...
    pub fn backend_from_request(
        ...
        let res = match (front_should_stick, sticky_session) {
            (true, Some(sticky_session)) => self
                .backends
                .borrow_mut()
                .backend_from_sticky_session(app_id, &sticky_session)
                .map_err(|e| {
                    debug!(
                        "Couldn't find a backend corresponding to sticky_session {} for app {}",
                        sticky_session, app_id
                    );
                    e
                }),
        ...
        match res {
            Err(e) => {
                let answer =
                    self.get_service_unavailable_answer(Some(app_id), &session.listen_token);
    ...
    fn app_id_from_request(&mut self, session: &mut Session) -> Result<String, ConnectionError> {
        ...
        let host: &str = if let Ok((i, (hostname, port))) = hostname_and_port(h.as_bytes()) {
            if i != &b""[..] {
                ...
                let answer = self.listeners[&session.listen_token]
                    .answers
                    .BadRequest
                    .clone();
    ...
    fn check_circuit_breaker(&mut self, session: &mut Session) -> Result<(), ConnectionError> {
        if session.connection_attempt == CONN_RETRIES {
            ...
            let answer = self.get_service_unavailable_answer(
                session.app_id.as_ref().map(|app_id| app_id.as_str()),
                &session.listen_token,
            );
            session.set_answer(DefaultAnswerStatus::Answer503, answer);
    ...
    fn get_service_unavailable_answer(
        &self,
        app_id: Option<&str>,
        listen_token: &Token,
    ) -> Rc<Vec<u8>> {
        if let Some(answer_service_unavailable) = app_id.and_then(|app_id| {
            self.custom_answers
                .get(app_id)
                .and_then(|c| c.ServiceUnavailable.as_ref())
        }) {
            answer_service_unavailable.clone()
        } else {
            self.listeners[&listen_token]
                .answers
                .ServiceUnavailable
                .clone()
        }
...
impl ProxyConfiguration<Session> for Proxy {
        ...
        if let Some(ref listener) = self.listeners.get(&Token(token.0)) {
            ...
            if let Ok(ssl) = Ssl::new(&listener.default_context) {
                ...
                let c = Session::new(
                    ...
                    listener.config.public_address,
    ...
    fn connect_to_backend(
        ...
        if (session.http().and_then(|h| h.app_id.as_ref()) == Some(&app_id))
        ...
        let front_should_stick = self
            ...
            .unwrap_or(false);
        let socket =
            self.backend_from_request(session, &app_id, front_should_stick, sticky_session)?;
        ...
        match message.order {
            ...
            ProxyRequestData::AddCertificate(add_certificate) => {
                if let Some(mut listener) = self
                    .listeners
                    .values_mut()
                    .find(|l| l.address == add_certificate.front)
                {
            ...
            ProxyRequestData::RemoveCertificate(remove_certificate) => {
                if let Some(mut listener) = self
                    .listeners
                    .values_mut()
                    .find(|l| l.address == remove_certificate.front)
                {
            ...
            ProxyRequestData::ReplaceCertificate(replace) => {
                if let Some(mut listener) = self
                    .listeners
                    .values_mut()
                    .find(|l| l.address == replace.front)
                {
            ...
            ProxyRequestData::Query(Query::Certificates(QueryCertificateType::Domain(d))) => {
                let res = self
                    ...
                    .map(|(addr, listener)| {
                        ...
                        (
                            listener.address,
                            domains.domain_lookup(d.as_bytes()).map(|(k, v)| {
...
pub fn start(config: HttpsListener, channel: ProxyChannel, max_buffers: usize, buffer_size: usize) {
    ...
    let pool = Rc::new(RefCell::new(Pool::with_capacity(
        2 * max_buffers,
        0,
        || Buffer::with_capacity(buffer_size),
    )));
    let backends = Rc::new(RefCell::new(BackendMap::new()));

    let mut sessions: Slab<Rc<RefCell<ProxySessionCast>>, SessionToken> =
        Slab::with_capacity(max_buffers);
    {
        let entry = sessions
            .vacant_entry()
            .expect("session list should have enough room at startup");
    ...
    let token = {
        let entry = sessions
            .vacant_entry()
            .expect("session list should have enough room at startup");
        let e = entry.insert(Rc::new(RefCell::new(ListenSession {
            protocol: Protocol::HTTPListen,
        })));
        Token(e.index().0)
    };
    ...
    if configuration.add_listener(config, token).is_some() {
        if configuration
            .activate_listener(&mut event_loop, &front, None)
            .is_some()
        {
...
mod tests {
    ...
    */
    ...
    fn frontend_from_request_test() {
        ...
        fronts.domain_insert(
            Vec::from(&b"lolcatho.st"[..]),
            vec![
                TlsApp {
                    app_id: app_id1,
