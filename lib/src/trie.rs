...
impl<V: Debug + Clone> TrieNode<V> {
                ...
                None => {
                    if partial_key.len() > child_key.len() {
                        return self.children[index]
                            .lookup_recursive(&partial_key[child_key.len()..]);
                    } else if partial_key.len() == child_key.len() {
                        return self.children[index].key_value.as_ref();
                    } else {
                        return None;
        ...
        for (index, ref child_key) in self.keys.iter().enumerate() {
            ...
            match pos {
                ...
                None => {
                    if partial_key.len() > child_key.len() {
                        return self.children[index]
                            .domain_lookup_recursive(&partial_key[child_key.len()..]);
                    } else if partial_key.len() == child_key.len() {
                        return self.children[index].key_value.as_ref();
                    } else {
                        return None;
    ...
    // specific version that will handle wildcard domains
    pub fn domain_lookup_mut_recursive(
        &mut self,
        partial_key: &[u8],
        ...
        for (index, ref child_key) in self.keys.iter().enumerate() {
            ...
            match pos {
                ...
                None => {
                    if partial_key.len() > child_key.len() {
                        return self.children[index]
                            .domain_lookup_mut_recursive(&partial_key[child_key.len()..]);
                    } else if partial_key.len() == child_key.len() {
...
mod tests {
    ...
    fn domains() {
        ...
        assert_eq!(
            root.domain_lookup(&b"www.example.com"[..]),
            Some(&((&b"www.example.com"[..]).to_vec(), 1))
        );
        assert_eq!(
            root.domain_lookup(&b"alldomains.org"[..]),
            Some(&((&b"alldomains.org"[..]).to_vec(), 4))
        );
        assert_eq!(
            root.domain_lookup(&b"test.hello.com"[..]),
            Some(&((&b"*.hello.com"[..]).to_vec(), 7))
        );
        assert_eq!(
            root.domain_lookup(&b"test.alldomains.org"[..]),
            Some(&((&b"*.alldomains.org"[..]).to_vec(), 3))
        );
        assert_eq!(
            root.domain_lookup(&b"hello.alldomains.org"[..]),
            Some(&((&b"*.alldomains.org"[..]).to_vec(), 3))
        );
        assert_eq!(
            root.domain_lookup(&b"pouet.alldomains.org"[..]),
