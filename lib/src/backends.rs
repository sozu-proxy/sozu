...
use sozu_command::{config::LoadBalancingAlgorithms, proxy};

use super::{load_balancing::*, AppId, Backend, ConnectionError};
...
impl BackendMap {
    ...
    pub fn import_configuration_state(&mut self, backends: &HashMap<AppId, Vec<proxy::Backend>>) {
        self.backends
            .extend(backends.iter().map(|(ref app_id, ref backend_vec)| {
                (
                    app_id.to_string(),
            ...
            if let Some(ref mut b) = app_backends.next_available_backend() {
                ...
                let res = conn.map(|c| (b.clone(), c)).map_err(|e| {
                    error!(
                        ...
                        app_id, backend.address, backend.failures
                    );
                    e
                });
...
impl BackendList {
    ...
    pub fn add_backend(&mut self, backend: Backend) {
        if self
            .backends
            .iter()
            .find(|b| (*b.borrow()).address == backend.address)
            .is_none()
        {
    ...
    pub fn set_load_balancing_policy(&mut self, load_balancing_policy: LoadBalancingAlgorithms) {
        match load_balancing_policy {
            LoadBalancingAlgorithms::RoundRobin => {
                self.load_balancing = Box::new(RoundRobinAlgorithm { next_backend: 0 })
            }
            LoadBalancingAlgorithms::Random => self.load_balancing = Box::new(RandomAlgorithm {}),
            LoadBalancingAlgorithms::LeastConnections => {
                self.load_balancing = Box::new(LeastConnectionsAlgorithm {})
            }
        }
    }
}
