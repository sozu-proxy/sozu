...
use sozu_e2e::{
    ...
    setup::{
        create_local_address, provide_port, repeat_until_error_or, setup_async_test,
        setup_sync_test, State,
    },
    sozu::worker::Worker,
};
...
fn try_backpressure(nb_clients: usize, payload_size: usize) -> State {
    let hostname = "localhost".to_string();
    let front_port = provide_port();
    let front_address: SocketAddr = format!("127.0.0.1:{}", front_port)
        .parse()
    ...
    // let mut client = Client::new("Terminator", front_address, "");
    // client.connect();
    // set.join_next();
    worker.hard_stop();
    worker.wait_for_server_stop();
    // let status = backend.join();
    // println!("{status:?}");
...
fn test_backpressure() {
    assert_eq!(
        repeat_until_error_or(1, "Backpressure", || try_backpressure(500, 0)),
        State::Success
    );
}
