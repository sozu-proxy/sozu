...
pub fn try_issue_810_panic(part2: bool) -> State {
    ...
    worker.send_proxy_request(RequestContent::AddTcpListener(
        Worker::default_tcp_listener(front_address),
    ));
    worker.send_proxy_request(RequestContent::ActivateListener(ActivateListener {
        address: front_address,
        proxy: ListenerType::TCP,
        from_scm: false,
    }));
    worker.send_proxy_request(RequestContent::AddCluster(Worker::default_cluster(
        "cluster_0",
    )));
    worker.send_proxy_request(RequestContent::AddTcpFrontend(
        Worker::default_tcp_frontend("cluster_0", front_address),
    ));

    worker.send_proxy_request(RequestContent::AddBackend(Worker::default_backend(
        "cluster_0",
        "cluster_0-0",
        back_address,
    )));
...
pub fn try_tls_endpoint() -> State {
    ...
    worker.send_proxy_request(RequestContent::AddHttpsListener(
    ...
    worker.send_proxy_request(RequestContent::AddHttpsFrontend(HttpFrontend {
    ...
    worker.send_proxy_request(RequestContent::AddCertificate(add_certificate));
...
pub fn test_upgrade() -> State {
    ...
    new_worker.send_proxy_request(RequestContent::SoftStop);
...
pub fn try_hard_or_soft_stop(soft: bool) -> State {
    ...
    if soft {
        ...
        worker.send_proxy_request(RequestContent::HardStop);
    }
    thread::sleep(Duration::from_millis(100));
...
fn try_http_behaviors() -> State {
    ...
    worker.send_proxy_request(RequestContent::AddHttpListener(
    ...
    worker.send_proxy_request(RequestContent::AddHttpFrontend(HttpFrontend {
    ...
    info!("expecting 200");
    worker.send_proxy_request(RequestContent::RemoveBackend(RemoveBackend {
