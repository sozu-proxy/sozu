...
use proxy::{
    ActivateListener, AddCertificate, Application, Backend, CertFingerprint, CertificateAndKey,
    HttpFront, HttpListener, HttpsListener, ListenerType, ProxyRequestData, QueryAnswerApplication,
    RemoveBackend, RemoveCertificate, RemoveListener, TcpFront, TcpListener,
};
...
pub struct ConfigState {
    ...
    pub certificates:
        HashMap<SocketAddr, HashMap<CertFingerprint, (CertificateAndKey, Vec<String>)>>,
    //ip, port
...
impl ConfigState {
        ...
        match order {
            ...
            &ProxyRequestData::AddHttpListener(ref listener) => {
                if self.http_listeners.contains_key(&listener.front) {
                    ...
                    self.http_listeners
                        .insert(listener.front, (listener.clone(), false));
                    true
                }
            }
            ...
            &ProxyRequestData::RemoveHttpFront(ref front) => {
                if let Some(front_list) = self.http_fronts.get_mut(&front.app_id) {
                    ...
                    front_list.retain(|el| {
                        el.hostname != front.hostname || el.path_begin != front.path_begin
                    });
            ...
            &ProxyRequestData::AddCertificate(ref add) => {
                ...
                {
                    self.certificates.get_mut(&add.front).unwrap().insert(
                        fingerprint.clone(),
            ...
            &ProxyRequestData::ReplaceCertificate(ref replace) => {
                ...
                if !self
                    ...
                    .contains_key(&fingerprint)
                {
                    self.certificates.get_mut(&replace.front).map(|certs| {
                        certs.insert(
                            fingerprint.clone(),
    ...
    pub fn diff(&self, other: &ConfigState) -> Vec<ProxyRequestData> {
        ...
        let added_apps: Vec<&Application> = their_apps
            .difference(&my_apps)
            .filter_map(|app_id| other.applications.get(app_id.as_str()))
            .collect();
        ...
        let added_tcp_listeners: Vec<&SocketAddr> = their_tcp_listeners
            .difference(&my_tcp_listeners)
            .cloned()
            .collect();
        ...
        let my_certificates: HashSet<(
            SocketAddr,
            &CertFingerprint,
            &(CertificateAndKey, Vec<String>),
        )> = HashSet::from_iter(self.certificates.iter().flat_map(|(addr, certs)| {
            certs
                .iter()
                .zip(repeat(*addr))
                .map(|((k, v), addr)| (addr, k, v))
        }));
        let their_certificates: HashSet<(
            SocketAddr,
            &CertFingerprint,
            &(CertificateAndKey, Vec<String>),
...
pub fn get_application_ids_by_domain(
    ...
    state.http_fronts.values().for_each(|fronts| {
        fronts
            .iter()
            .filter(|front| {
                domain_check(&front.hostname, &front.path_begin, &hostname, &path_begin)
            })
            .for_each(|front| {
                app_ids.insert(front.app_id.clone());
            });
    });

    state.https_fronts.values().for_each(|fronts| {
        fronts
            .iter()
            .filter(|front| {
                domain_check(&front.hostname, &front.path_begin, &hostname, &path_begin)
...
pub fn get_certificate(state: &ConfigState, fingerprint: &[u8]) -> Option<(String, Vec<String>)> {
    state
        .certificates
        .values()
        .filter_map(|h| h.get(&CertFingerprint(fingerprint.to_vec())))
...
mod tests {
    ...
    use proxy::{Backend, HttpFront, LoadBalancingParams, ProxyRequestData};

    #[test]
    fn serialize() {
        let mut state: ConfigState = Default::default();
        state.handle_order(&ProxyRequestData::AddHttpFront(HttpFront {
    ...
    fn diff() {
        ...
        state.handle_order(&ProxyRequestData::AddApplication(Application {
            app_id: String::from("app_2"),
            sticky_session: true,
            https_redirect: true,
            proxy_protocol: None,
        ...
        state2.handle_order(&ProxyRequestData::AddHttpFront(HttpFront {
            app_id: String::from("app_1"),
            hostname: String::from("lolcatho.st:8080"),
            path_begin: String::from("/"),
            address: "0.0.0.0:8080".parse().unwrap(),
        ...
        state2.handle_order(&ProxyRequestData::AddApplication(Application {
            app_id: String::from("app_3"),
