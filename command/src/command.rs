...
use proxy::{AggregatedMetricsData, ProxyEvent, ProxyRequestData, QueryAnswer};
                ...
                E: serde::de::Error,
            {
                match value {
                    ...
                    e => Err(serde::de::Error::custom(format!(
                        "expected id, version, worker id, type or data, got: {}",
                        e
                    ))),
                }
            }
        ...
        V: serde::de::MapAccess<'de>,
    {
        ...
        loop {
            match try!(visitor.next_key()) {
                Some(CommandRequestField::Type) => {
                    config_type = Some(try!(visitor.next_value()));
                }
                Some(CommandRequestField::Id) => {
                    id = Some(try!(visitor.next_value()));
                }
                Some(CommandRequestField::Version) => {
                    version = Some(try!(visitor.next_value()));
                }
                Some(CommandRequestField::WorkerId) => {
                    worker_id = Some(try!(visitor.next_value()));
                }
                Some(CommandRequestField::Data) => {
        ...
        let data = if config_type == "PROXY" {
            ...
            let command = try!(serde_json::from_value(data)
                .or_else(|_| Err(serde::de::Error::custom("proxy configuration command"))));
            ...
            let state: SaveStateData = try!(serde_json::from_value(data)
                .or_else(|_| Err(serde::de::Error::custom("save state"))));
            ...
            CommandRequestData::LaunchWorker(try!(serde_json::from_value(data)
                .or_else(|_| Err(serde::de::Error::custom("launch worker")))))
            ...
            CommandRequestData::UpgradeWorker(try!(serde_json::from_value(data)
                .or_else(|_| Err(serde::de::Error::custom("upgrade worker")))))
...
mod tests {
    ...
    use hex::FromHex;
    use proxy::{
        AddCertificate, AppMetricsData, Application, Backend, CertFingerprint, CertificateAndKey,
        FilteredData, HttpFront, LoadBalancingParams, MetricsData, Percentiles, ProxyRequestData,
        RemoveBackend, RemoveCertificate,
    };
    use serde_json;
    ...
    fn config_message_test() {
        ...
        assert_eq!(
            message.data,
            CommandRequestData::Proxy(ProxyRequestData::AddHttpFront(HttpFront {
    ...
    test_message!(
        remove_application,
        "../assets/remove_application.json",
        CommandRequest {
            id: "ID_TEST".to_string(),
            version: 0,
            data: CommandRequestData::Proxy(ProxyRequestData::RemoveApplication(String::from(
                "xxx"
            ))),
            worker_id: None
        }
    ...
    test_message!(
        ...
        CommandRequest {
            id: "ID_TEST".to_string(),
            version: 0,
            data: CommandRequestData::Proxy(ProxyRequestData::RemoveCertificate(
                RemoveCertificate {
                    front: "0.0.0.0:443".parse().unwrap(),
