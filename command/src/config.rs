...
use proxy::{
    ActivateListener, AddCertificate, Application, Backend, CertificateAndKey, HttpFront,
    HttpListener, HttpsListener, ListenerType, LoadBalancingParams, ProxyRequestData, TcpFront,
    TcpListener, TlsProvider, TlsVersion,
};

use command::{CommandRequest, CommandRequestData, PROTOCOL_VERSION};
...
impl Listener {
    ...
    pub fn to_tls(&self) -> Option<HttpsListener> {
        ...
        let supported_ciphersuites: HashSet<&str> = [
    ...
    pub fn generate_config_messages(&self) -> Vec<CommandRequest> {
        ...
        for listener in &self.http_listeners {
            v.push(CommandRequest {
                ...
                worker_id: None,
                data: CommandRequestData::Proxy(ProxyRequestData::AddHttpListener(
                    listener.clone(),
                )),
            });
            count += 1;
        ...
        for listener in &self.https_listeners {
            v.push(CommandRequest {
                ...
                version: PROTOCOL_VERSION,
                worker_id: None,
                data: CommandRequestData::Proxy(ProxyRequestData::AddHttpsListener(
                    listener.clone(),
                )),
            });
            count += 1;
        ...
        if self.activate_listeners {
            for listener in &self.http_listeners {
                v.push(CommandRequest {
                    ...
                    version: PROTOCOL_VERSION,
                    worker_id: None,
                    data: CommandRequestData::Proxy(ProxyRequestData::ActivateListener(
                        ActivateListener {
