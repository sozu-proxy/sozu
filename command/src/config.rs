...
impl AppConfig {
    pub fn load_from_path(path: &str) -> io::Result<AppConfig> {
        let data = try!(ConfigUtils::load_file(path));
    ...
    pub fn load_apps(dir_path: &str) -> io::Result<Vec<AppConfig>> {
        let config_files = ConfigUtils::list_dir_files(dir_path)?;
        //warn on unacceptable files
        let mut apps = Vec::<AppConfig>::new();
        for path in config_files {
             match AppConfig::load_from_path(path.as_str()) {
...
impl ConfigUtils {
    ...
    pub fn list_dir_files(path: &str) -> io::Result<Vec<String>> {
...
impl Config {
    ...
    pub fn generate_config_messages(&self, apps: &Vec<AppConfig>) -> Vec<ConfigMessage> {
        let mut v = Vec::new();
        let mut count = 0u8;
        for app in apps {
            ...
            let key_opt = app.key.as_ref().and_then(|path| ConfigUtils::load_file(&path).ok());
            let certificate_opt = app.certificate.as_ref().and_then(|path| ConfigUtils::load_file(&path).ok());
            let chain_opt = app.certificate_chain.as_ref().and_then(|path| ConfigUtils::load_file(&path).ok())
                .map(split_certificate_chain);

            //create the front both for HTTP and HTTPS if possible
            let order = Order::AddHttpFront(HttpFront {
                app_id: app.appname.clone(),
...
mod tests {
    ...
    #[test]
    fn list_dir_files() {
        match ConfigUtils::list_dir_files("/Users/ssarabadani/projects/rust/sozu/command/assets/apps.d") {
    ...
    #[test]
    fn load_app_configs() {
        match AppConfig::load_apps("/Users/ssarabadani/projects/rust/sozu/command/assets/apps.d") {
            Ok(configs) => assert_eq!(configs.len(),2),
            Err(e) => println!("{}",e)
        }
    }
