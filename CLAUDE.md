...
- **Logging**: every protocol module defines its own `macro_rules! log_context!` / `log_context_lite!` / `log_module_context!` (see `protocol/mux/mod.rs:49/87/118`, `protocol/rustls.rs:23`, `protocol/pipe.rs:26`, `tcp.rs:68`). Prefix tags `MUX`, `MUX-H1`, `MUX-H2`, `RUSTLS`, `SOCKET`, `PIPE` are load-bearing for log-search. Use the macros — do NOT call `log::info!`/`log::error!` directly from protocol code.
